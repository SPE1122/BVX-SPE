...
def _manual_reflow_zone_platform(manual_df: pd.DataFrame, platform_row: Optional[pd.Series]) -> pd.DataFrame:
    ...
    for zone in _MANUAL_ZONE_ORDER:
        ...
        for idx, row in zrows.iterrows():
            ...
            # X-Position: Wenn nur eine Zone in dieser Seite aktiv ist, sauber mittig in X.
            # Wenn vorne + hinten aktiv sind, fachlich vorne/hinten setzen.
            if not both_front_back:
                x_new = max(0.0, (eff_len - length) / 2.0) if eff_len > 0 else 0.0
            elif fb == 'vorne':
                x_new = max(0.0, eff_len - length) if eff_len > 0 else 0.0
            else:
                x_new = 0.0
...
def _manual_assign_unit_to_zone(
    ...
    return _manual_reflow_all_zones(result, platforms_df)
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v41')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v41') if option_choices_manual else ''
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                top1, top2, top3 = st.columns([1.4, 1.0, 0.8])
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v41')
                default_zone_label = top2.radio(
                    'Zielzone',
                    ['vorne links', 'vorne rechts', 'hinten links', 'hinten rechts'],
                    horizontal=True,
                    key='manual_zone_target_v41',
                )
                if top3.button('Reset', key='manual_zone_reset_v41'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v41')
                    ...
                    if work_df.empty:
                        ...
                        def _unit_choice_label(r: pd.Series) -> str:
                            ...
                            return f"{r.get('Einheit_ID','')} | {b} | {status}"
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v41')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v41',
                        )

                        z1, z2 = st.columns(2)
                        if z1.button(f'→ {default_zone_label}', key='manual_zone_apply_default_v41', type='primary'):
                            manual_df = _manual_assign_unit_to_zone(
                                manual_df,
                                platforms_used_df,
                                selected_unit_id,
                                selected_platform_row,
                                _manual_zone_from_label(default_zone_label),
                                float(spacer_under),
                            )
                            st.session_state['manual_placements_df'] = clean_placements_dataframe(manual_df)
                            st.rerun()

                        if z2.button('entladen', key='manual_zone_unload_v41'):
                            manual_df = _manual_unload_unit(manual_df, platforms_used_df, selected_unit_id)
                            st.session_state['manual_placements_df'] = clean_placements_dataframe(manual_df)
                            st.rerun()

                        st.markdown('**Direkt-Zone**')
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v41'):
                                manual_df = _manual_assign_unit_to_zone(
                                    manual_df,
                                    platforms_used_df,
                                    selected_unit_id,
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v41',
                        )
                        platform_changed = (
                        ...
                        if platform_changed:
                            ...
                            st.session_state['manual_placements_df'] = _manual_reflow_all_zones(manual_df, platforms_used_df)
                            st.rerun()
                ...
                with c_right:
                    ...
                    if selected_platform_row is not None:
                        ...
                        st.info(
                            f"Max. Breite Pritsche: {safe_number(selected_platform_row.get('Breite_mm')):.0f} mm · "
                            f"max. Ladehöhe: {safe_number(selected_platform_row.get('Max_Höhe_mm')):.0f} mm · "
                            f"aktuelle Ladehöhe: {used_height:.0f} mm · "
                            f"aktuelle Breite belegt: {used_width:.0f} mm"
                        )
                    ...
                    st.write(f'Verladen: {loaded_now} · offen: {free_now}')
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v41',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v41')
                        if create_view:
                            manual_view_df = _manual_reflow_all_zones(st.session_state.get('manual_placements_df', manual_df), platforms_used_df)
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v41_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                            st.caption('Noch keine Ansicht erstellt.')

                # Schneller manueller Modus:
                # Keine automatische Ansichtserstellung und kein unnötiges Reflow bei jedem Klick.
                # Reflow passiert direkt beim Zonen-Klick oder bei geänderten Pritschenwerten.
