...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v38')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v38') if option_choices_manual else ''
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            st.info('Automatik ist aktiv. Manuelle Tabelle ist nur aktiv, wenn oben „Manuell“ gewählt wird.')
            st.dataframe(edited_placements_df, use_container_width=True, hide_index=True)
        else:
            st.markdown('**Manuelle Verladung – einfacher Modus**')
            st.caption('Ein Formular: Pritsche wählen, Zeilen anhaken, Position/Lage/Einlage setzen, dann einmal übernehmen. Keine dauernde Live-Neuberechnung.')
            ...
            if not platform_names:
                ...
                top1, top2, top3 = st.columns([1.4, 1.0, 1.0])
                selected_manual_platform = top1.selectbox('Pritsche wählen', platform_names, key='manual_simple_platform_v38')
                show_sketch = top2.checkbox('Skizze anzeigen', value=False, key='manual_simple_show_sketch_v38')
                if top3.button('Zurücksetzen', key='manual_simple_reset_v38'):
                    manual_init = clean_placements_dataframe(placements_df)
                    for col in ['Einlage_unten_mm', 'Einlage_oben_mm']:
                        manual_init[col] = 0.0
                    st.session_state['manual_placements_df'] = manual_init
                    st.session_state['manual_platforms_df'] = _clean_manual_platforms_dataframe(platforms_used_df)
                    st.rerun()
                ...
                with st.expander('Pritschenwerte ändern', expanded=False):
                    ...
                    edited_platform = st.data_editor(
                        ...
                        hide_index=True,
                        num_rows='fixed',
                        key='manual_platform_values_v38',
                    )
                    if not edited_platform.empty:
                ...
                # Arbeitsliste: unverladene Einheiten + Einheiten auf aktuell gewählter Pritsche.
                ...
                if work_df.empty:
                    st.warning('Keine Bauteile/Bunde für diese Auswahl vorhanden.')
                else:
                    search_text = st.text_input('Suche Bauteil/Bund', value='', key='manual_search_v38')
                    ...
                    editor = work_df.copy()
                    editor['Auswahl'] = False
                    editor['Ziel_Pritsche'] = selected_manual_platform
                    editor['Position'] = editor.get('Manuelle_Position', 'vorne links')
                    editor['Lage'] = editor.get('Manuelle_Lage', 'ganz unten')
                    if 'Z_manuell_mm' not in editor.columns:
                        editor['Z_manuell_mm'] = 0.0

                    display_cols = [
                        'Auswahl', 'Einheit_ID', 'Typ', 'Bauteile', 'Pritsche',
                        'Ziel_Pritsche', 'Position', 'Lage',
                        'Einlage_unten_mm', 'Einlage_oben_mm', 'Z_manuell_mm',
                        'Länge_mm', 'Breite_mm', 'Höhe_mm', 'Gewicht_kg'
                    ]
                    display_cols = [c for c in display_cols if c in editor.columns]

                    with st.form('manual_batch_form_v38'):
                        edited_batch = st.data_editor(
                            editor[display_cols],
                            use_container_width=True,
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_batch_editor_v38',
                            column_config={
                                'Auswahl': st.column_config.CheckboxColumn('Auswahl'),
                                'Ziel_Pritsche': st.column_config.SelectboxColumn('Ziel-Pritsche', options=platform_names),
                                ...
                                'Lage': st.column_config.SelectboxColumn(
                                    'Lage',
                                    options=['behalten', 'ganz unten', 'eine Lage höher', 'eine Lage tiefer', 'Z manuell'],
                                ),
                                'Einlage_unten_mm': st.column_config.NumberColumn('Einlage unten mm'),
                                'Einlage_oben_mm': st.column_config.NumberColumn('Einlage oben mm'),
                                'Z_manuell_mm': st.column_config.NumberColumn('Z manuell mm'),
                            },
                            disabled=['Einheit_ID', 'Typ', 'Bauteile', 'Pritsche', 'Länge_mm', 'Breite_mm', 'Höhe_mm', 'Gewicht_kg'],
                        )
                        apply_manual = st.form_submit_button('Änderungen übernehmen', type='primary')

                    if apply_manual:
                        changed = 0
                        for _, erow in edited_batch.iterrows():
                            if not bool(erow.get('Auswahl', False)):
                                continue
                            unit_id = str(erow.get('Einheit_ID', ''))
                            match_idx = manual_df.index[manual_df.get('Einheit_ID', pd.Series(dtype=str)).astype(str).eq(unit_id)].tolist()
                            if not match_idx:
                                continue
                            idx = match_idx[0]
                            target_platform = str(erow.get('Ziel_Pritsche') or selected_manual_platform)
                            p_row = _manual_platform_row(platforms_used_df, target_platform)
                            manual_df = _manual_place_by_position(
                                manual_df,
                                idx,
                                p_row,
                                position=str(erow.get('Position') or 'vorne links'),
                                layer_action=str(erow.get('Lage') or 'ganz unten'),
                                spacer_under=safe_number(erow.get('Einlage_unten_mm'), 0.0),
                                custom_z=safe_number(erow.get('Z_manuell_mm'), 0.0),
                            )
                            if 'Einlage_oben_mm' not in manual_df.columns:
                                manual_df['Einlage_oben_mm'] = 0.0
                            if 'Manuelle_Position' not in manual_df.columns:
                                manual_df['Manuelle_Position'] = ''
                            if 'Manuelle_Lage' not in manual_df.columns:
                                manual_df['Manuelle_Lage'] = ''
                            if 'Z_manuell_mm' not in manual_df.columns:
                                manual_df['Z_manuell_mm'] = 0.0
                            manual_df.at[idx, 'Einlage_oben_mm'] = safe_number(erow.get('Einlage_oben_mm'), 0.0)
                            manual_df.at[idx, 'Manuelle_Position'] = str(erow.get('Position') or '')
                            manual_df.at[idx, 'Manuelle_Lage'] = str(erow.get('Lage') or '')
                            manual_df.at[idx, 'Z_manuell_mm'] = safe_number(erow.get('Z_manuell_mm'), 0.0)
                            changed += 1
                        st.session_state['manual_placements_df'] = clean_placements_dataframe(manual_df)
                        st.success(f'{changed} Zeile(n) übernommen.')
                        st.rerun()

                # Alles nach manueller Eingabe nur einmal neu berechnen.
                ...
                if show_sketch:
                    ...
                    st.plotly_chart(
                        ...
                        use_container_width=True,
                        key=f'manual_plot_v38_{selected_manual_platform}_{view_map[view_choice]}',
                    )
                else:
                    st.caption('Skizze ist ausgeschaltet. Dadurch bleibt der manuelle Modus schneller.')

                if not_loaded_count:
                    st.warning('Es sind noch Einheiten nicht verladen.')
