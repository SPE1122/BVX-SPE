...
def create_bsd_matrix_for_platform(
    ...
    - Keine starre Drittel-Logik: Mitte entsteht nur bei echten 3 X-Stapeln je Seite.
    - Die Tabelle ist unten ausgerichtet: unten ist unten, Kantholz bleibt unten.
    - Jede Positionsspalte ist ein eigener Stapel; keine gemeinsame Z-Höhenmatrix.
    ...
    def _cluster_positions_for_layer(items: List[Tuple[int, float]]) -> Dict[int, str]:
        ...
        if len(clusters) == 1:
            # Nur eine X-Gruppe: keine Mitte erzwingen.
            # Bei x-mittig gilt weiterhin vorne.
            for idx in order:
                # Dummy row gibt es hier nicht; direkt über center entscheiden.
                center = centers[idx]
                if front_at_x_max:
                    label_by_cluster[idx] = 'Vorne' if center >= eff_length / 2.0 else 'Hinten'
                else:
                    label_by_cluster[idx] = 'Vorne' if center <= eff_length / 2.0 else 'Hinten'
        elif len(clusters) == 2:
            if front_at_x_max:
    ...
    def _build_x_zone_lookup() -> Dict[Tuple[int, str], str]:
        """Erkennt feste X-Stapel je Seite.

        Wichtig V60:
        Die Zuordnung hinten/mitte/vorne wird NICHT je Z-Lage neu berechnet.
        Pro Seite werden zuerst stabile X-Stapel gebildet. Der ganze Stapel bleibt
        dann in derselben BSD-Spalte. Dadurch springen Bunde optisch nicht mehr
        zwischen vorne/hinten/mitte.
        """
        ...
        for ridx, r in rows.iterrows():
            ...
            for side in _side_names(r):
                groups.setdefault(side, []).append((ridx, x_mid))
    ...
    # V60: Keine gemeinsame Z-Höhenmatrix mehr.
    # Jede 6er-Position wird als eigener Stapel unten ausgerichtet.
    # Einlagen bleiben nur innerhalb ihres Stapels und erzeugen keine waagerechten Unterbrüche
    # in Nachbarstapeln.
    slot_lists_bottom_to_top: Dict[str, List[Dict[str, Any]]] = {}
    for slot in position_cols:
        slot_entries = [e for e in entries if e['slot'] == slot]
        slot_entries = sorted(
            slot_entries,
            key=lambda e: (round(float(e.get('z', 0.0)), 1), str(e.get('label', ''))),
            reverse=False
        )
        slot_lists_bottom_to_top[slot] = slot_entries

    max_len = max((len(v) for v in slot_lists_bottom_to_top.values()), default=0)
    matrix_rows: List[Dict[str, Any]] = []

    for display_idx in range(max_len):
        # display_idx 0 = oberste Tabellenzeile.
        # Kürzere Stapel bekommen nur oben Leerfelder; unten bleibt unten.
        out: Dict[str, Any] = {
            'Pritsche': pname,
            'Fuhre_Nr': platform.get('Fuhre_Nr', ''),
            'Lage': max_len - display_idx,
            'Z_mm': '',
            'Höhe_mm': '',
            'Breite_mm': '',
            'Gesamtlänge_mm': '',
            'Gewicht_kg': '',
            'Anzahl_Einheiten': '',
            'Zeilentyp': 'BSD-6-Spalten-Stapelliste',
        }
        ...
        for slot in position_cols:
            stack = slot_lists_bottom_to_top[slot]  # unten -> oben
            top_pad = max_len - len(stack)
            local_idx = display_idx - top_pad
            if local_idx < 0 or local_idx >= len(stack):
                continue

            # Anzeige oben -> unten; stack bleibt intern unten -> oben.
            e = stack[len(stack) - 1 - local_idx]
            out[slot] = str(e.get('label', '') or '')
            if e.get('remark'):
                out[f'Bemerkung {slot.lower()}'] = str(e.get('remark', '') or '')
...
def _pdf_draw_bsd_matrix_page(c, page_w: float, page_h: float, margin: float, platform: pd.Series, matrix_df: pd.DataFrame, header: Dict[str, Any], project_name: str, logo_bytes: Optional[bytes] = None) -> None:
    """Zeichnet eine BSD-Seite als 6 unabhängige Stapelkästen.

    V60:
    - Keine normale Tabelle mehr.
    - Keine gemeinsamen horizontalen Linien über alle Spalten.
    - Jede Position ist ein eigener Stapel mit eigenen Boxen.
    - Einlagen unterbrechen nur den eigenen Stapel, nicht den Nachbarstapel.
    """
    from reportlab.lib import colors
    ...
    c.drawString(left_x + 8, top_y - 120, 'Darstellung: 6 einzelne Stapelkästen, unten = unten. Keine gemeinsamen waagerechten Tabellenlinien.')
    ...
    # Aus der bestehenden BSD-Matrix nur die Spalteninhalte extrahieren.
    # Wichtig: Die Linien werden danach spaltenweise gezeichnet, nicht mehr zeilenweise.
    stacks_top_to_bottom: Dict[str, List[str]] = {}
    for col in position_cols:
        vals: List[str] = []
        if col in rows.columns:
            for value in rows[col].tolist():
                ...
                if not text_value:
                    continue
                # Mehrzeilige Zelle als einzelne Boxen behandeln.
                for part in text_value.split('\n'):
                    part = str(part or '').strip()
                    if part:
                        vals.append(part)
        stacks_top_to_bottom[col] = vals

    max_count = max((len(v) for v in stacks_top_to_bottom.values()), default=0)
    if max_count <= 0:
        c.setFont('Helvetica', 8)
        c.drawString(margin, top_y - 160, 'Keine Ladeplan-BSD-Daten vorhanden')
        return
    ...
    area_top = top_y - 155
    area_bottom = 34
    area_h = max(80, area_top - area_bottom)
    gap = 10
    col_w = (page_w - 2 * margin - gap * (len(position_cols) - 1)) / len(position_cols)
    header_h = 22
    stack_h = area_h - header_h - 8
    # Nicht höher als 22 pt, sonst wird der Plan zu lang. Bei vielen Lagen automatisch kleiner.
    box_h = min(22.0, max(9.0, stack_h / max_count))
    usable_stack_h = box_h * max_count
    stack_bottom = area_bottom
    stack_top = stack_bottom + usable_stack_h

    def _draw_entry_text(x0: float, y0: float, w0: float, h0: float, txt: str) -> None:
        txt = str(txt or '').strip()
        if not txt:
            return
        # Schrift je nach Boxhöhe.
        fs = 6.2 if h0 >= 12 else 5.2
        c.setFont('Helvetica', fs)
        max_chars = max(8, int(w0 / (fs * 0.55)))
        lines = [txt[i:i + max_chars] for i in range(0, len(txt), max_chars)][:2]
        y_text = y0 + h0 / 2.0 + (len(lines) - 1) * fs * 0.35 - fs * 0.35
        for li, line in enumerate(lines):
            c.drawCentredString(x0 + w0 / 2.0, y_text - li * (fs + 1), line)
    ...
    for ci, col in enumerate(position_cols):
        ...
        c.drawCentredString(x0 + col_w / 2.0, area_top - 14, col)

        # Äusserer Stapelrahmen ohne interne waagerechte Linien über andere Spalten.
        c.setFillColor(colors.white)
        c.setStrokeColor(colors.black)
        c.setLineWidth(0.5)
        c.rect(x0, stack_bottom, col_w, usable_stack_h, stroke=1, fill=0)

        stack_top_to_bottom = stacks_top_to_bottom.get(col, [])
        # Unten ausrichten: top_to_bottom umdrehen und von unten nach oben zeichnen.
        stack_bottom_to_top = list(reversed(stack_top_to_bottom))
        y0 = stack_bottom
        for entry in stack_bottom_to_top:
            is_spacer = 'Einlage' in entry or 'Lagenholz' in entry
            is_timber = 'Kantholz' in entry
            if is_timber:
                c.setFillColor(colors.HexColor('#d9d9d9'))
            elif is_spacer:
            ...
            c.setStrokeColor(colors.black if is_timber else colors.darkgrey)
            c.setLineWidth(0.35)
            c.rect(x0, y0, col_w, box_h, stroke=1, fill=1)
            c.setFillColor(colors.black)
            _draw_entry_text(x0 + 2, y0 + 1, col_w - 4, box_h - 2, entry)
            y0 += box_h

    # Kleine Legende unten.
    c.setFillColor(colors.black)
    c.setFont('Helvetica', 6.2)
    c.drawString(margin, 18, 'Hinweis: Jede Spalte ist ein eigener Stapel. Einlagen/Kantholz gelten nur in der jeweiligen Spalte; keine gemeinsamen Tabellenzeilen.')
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v60')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v60') if option_choices_manual else ''
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v60', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v60')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v60'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v60'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v60'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v60', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v60', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v60')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v60')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v60',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v60', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v60', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v60',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v60')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v60', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v60', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v60', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v60'] = validation_df
                            st.session_state['manual_plan_ready_v60'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v60'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v60'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v60', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v60',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v60')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v60', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v60_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v60', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Die Tabelle wird automatisch für jede belegte Pritsche erstellt. V60: 6 Positionsspalten, unten ausgerichtet; Kantholz unten, leere Felder nur oben.')
