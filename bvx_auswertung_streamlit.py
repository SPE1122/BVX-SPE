...
def sort_parts_dataframe(
    ...
    second_attr: str,
    second_asc: bool,
    extra_sort_attrs: Optional[List[Tuple[str, bool]]] = None,
) -> pd.DataFrame:
    """Sortiert Bauteile mit Hauptattribut und mehreren Nebenattributen.

    V84:
    Zusätzlich zum bisherigen Nebenattribut können weitere Nebenattribute
    nacheinander ergänzt werden. Reihenfolge bleibt stabil und eindeutig.
    """
    sort_cols = []
    sort_ascending = []

    def add_sort(attr: str, asc: bool) -> None:
        if attr and attr != 'Keine' and attr in df.columns and attr not in sort_cols:
            sort_cols.append(attr)
            sort_ascending.append(bool(asc))

    add_sort(main_attr, main_asc)
    add_sort(second_attr, second_asc)
    for attr, asc in (extra_sort_attrs or []):
        add_sort(attr, asc)
...
def _support_width_ratio_for_candidate(state: Dict[str, Any], x: float, y: float, z: float, length: float, width: float) -> float:
    ...
    if float(z) <= base_z + 0.1:
        return float(_base_support_metrics_for_candidate(state, x, y, length, width).get('area_ratio', 0.0))
...
def _platform_physical_support_rect_from_state(state: Dict[str, Any]) -> Tuple[float, float, float, float]:
    """V117: physische Pritsche als echte untere Auflagefläche.

    X-Koordinaten der Verladung laufen über den effektiven Raum
    (Überhang hinten + Pritschenlänge + Überhang vorne). Die Pritsche selbst
    trägt aber nur zwischen Überhang_hinten und Überhang_hinten + Länge_mm.
    Kantholz darf dadurch nicht im freien Überhang als Auflage zählen.
    """
    base_len = max(0.0, float(state.get('Länge_mm', 0.0) or 0.0))
    back_allow = max(0.0, float(state.get('Überhang_hinten_mm', 0.0) or 0.0))
    platform_width = max(0.0, float(state.get('Breite_mm', 0.0) or 0.0))
    return back_allow, back_allow + base_len, 0.0, platform_width


def _base_support_metrics_for_candidate(state: Dict[str, Any], x: float, y: float, length: float, width: float) -> Dict[str, float]:
    """V117: Auflage auf der Pritsche/Kantholz real begrenzen.

    Die erste Lage darf überhängen, aber der Überhang ist nicht getragen.
    Deshalb wird als Basisauflage nur der Schnitt mit der physischen Pritsche
    bewertet. Das korrigiert den alten Fehler, dass Kantholz wie eine
    durchgehende Ebene über die ganze Ladung wirkte.
    """
    x0, x1 = float(x), float(x) + float(length)
    y0, y1 = float(y), float(y) + float(width)
    px0, px1, py0, py1 = _platform_physical_support_rect_from_state(state)
    ox0, ox1 = max(x0, px0), min(x1, px1)
    oy0, oy1 = max(y0, py0), min(y1, py1)
    footprint = max(1.0, float(length) * float(width))
    if ox1 <= ox0 or oy1 <= oy0:
        return {'area_ratio': 0.0, 'free_length_mm': max(0.0, float(length)), 'free_side_mm': max(0.0, float(width))}
    overlap_area = (ox1 - ox0) * (oy1 - oy0)
    return {
        'area_ratio': max(0.0, min(1.0, overlap_area / footprint)),
        'free_length_mm': _edge_free_span_mm(x0, x1, [(ox0, ox1)]),
        'free_side_mm': _edge_free_span_mm(y0, y1, [(oy0, oy1)]),
    }
...
def _edge_free_span_mm(total0: float, total1: float, intervals: List[Tuple[float, float]]) -> float:
    ...
    supported_start = min(a for a, _b in clean)
    supported_end = max(b for _a, b in clean)
...
def _support_metrics_for_candidate(state: Dict[str, Any], x: float, y: float, z: float, length: float, width: float) -> Dict[str, float]:
    ...
    - free_side_mm: grösster freier Randüberhang links/rechts
    """
    length = float(length)
    width = float(width)
    footprint = max(1.0, length * width)
    base_z = float(state.get('base_wood_height', 0.0))
    if float(z) <= base_z + 0.1:
        return _base_support_metrics_for_candidate(state, x, y, length, width)
    ...
    for r in rows:
        ...
        if ox1 > ox0 and oy1 > oy0:
            overlaps.append((ox0, ox1, oy0, oy1))
            x_intervals.append((ox0, ox1))
            y_intervals.append((oy0, oy1))
...
def can_place_stable(state: Dict[str, Any], unit: pd.Series, x: float, y: float, z: float, length: float, width: float, height: float, weight: float) -> bool:
    ...
    # V117: Auch die erste Lage wird gegen die physische Pritsche geprüft.
    # Kantholz zählt nicht mehr als fiktive Auflage im freien Überhang.
    # Die Prüfung bleibt nur aktiv, wenn der Anwender die Auflage-Sperre einschaltet.
...
def create_loading_excel(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        font_head = Font(name='Arial', size=9, bold=True)
        font_body = Font(name='Arial', size=8)
        font_small = Font(name='Arial', size=7)
        ...
        def set_bsd_matrix_cell(cell, value: Any, is_einlage_row: bool = False):
            """Nur Excel-BSD.
            ...
            cell.value = txt
            low = txt.lower()
            if is_einlage_row or low.startswith('einlage'):
                cell.font = Font(name='Arial', size=7, bold=False)
                cell.fill = fill_white
                cell.border = Border()
            elif low.startswith('kantholz'):
                cell.font = Font(name='Arial', size=7, bold=False)
                cell.fill = fill_gray
                cell.border = border_dotted
            else:
        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            # V84: BSD-Matrix A:F breiter für bessere Lesbarkeit auf Ausdruck.
            widths = {
            ...
            # Matrixdaten an den unteren Teil setzen.
            # Nur Excel-BSD geändert:
            # Variante B: Einlageholz als kleine Zusatzzeile direkt unter dem Bundblock.
            _bsd_cols, _bsd_rows = _bsd_display_matrix_rows(matrix)
            write_start = data_row + max(0, max_rows - len(_bsd_rows) - 1)
            data_write_start = write_start
            data_write_end = write_start + len(_bsd_rows) - 1
            if _bsd_rows:
                prev_row_had_einlage = False
                for i, row_values in enumerate(_bsd_rows, start=write_start):
                    multiline = any('\n' in str(v or '') for v in row_values)
                    is_einlage_row = _bsd_excel_is_einlage_row(row_values)
                    for c, value in enumerate(row_values, start=1):
                        cell = ws.cell(i, c)
                        set_bsd_matrix_cell(cell, value, is_einlage_row=is_einlage_row)
                    if is_einlage_row:
                        ws.row_dimensions[i].height = 11
                    else:
                        ws.row_dimensions[i].height = 32 if multiline else 26
            ...
            # Nur Excel-BSD:
            # Alle übrigen beschrifteten Bereiche 13 pt.
            # Matrix-Bauteile bleiben 15 pt, Einlage-Zusatzzeile bleibt 7 pt.
            try:
                ...
                for row in ws.iter_rows(min_row=1, max_row=lower_top + 6, min_col=1, max_col=16):
                    for cell in row:
                        ...
                        in_matrix_data = (
                            1 <= int(cell.column) <= 6
                            and int(data_write_start) <= int(cell.row) <= int(data_write_end)
                        )
                        if in_matrix_data:
                            continue
                        new_font = _copy_font(cell.font)
                        new_font.sz = 13
                        if not new_font.name:
                            new_font.name = 'Arial'
                        cell.font = new_font
            except Exception:
                pass
...
def create_bsd_matrix_for_platform(
    ...
    V84:
    - 6 Positionsspalten: hinten/mitte/vorne je links/rechts.
    - Keine starre Drittel-Logik: Mitte entsteht nur bei 3 X-Gruppen in derselben Bundreihe/Seite.
    - Die Tabelle ist unten ausgerichtet: unten ist unten, Kantholz bleibt unten.
    - Jede Positionsspalte ist ein eigener Stapel; keine gemeinsamen Linien im PDF.
    - Leere Felder entstehen nur oben, wenn ein Stapel weniger Einträge hat.
    ...
    slot_has_load = {slot: False for slot in position_cols}
    ...
    def _row_y_mid(row: pd.Series) -> Optional[float]:
        width = safe_number(row.get('Breite_mm'))
        y0 = safe_number(row.get('Y_mm'))
        if width <= 0:
            return None
    ...
    def _fallback_front_back_by_x(row: pd.Series) -> str:
        """Fallback ohne Mitte: X-Mitte entscheidet vorne/hinten; X-mittig = vorne."""
        x_mid = _row_x_mid(row)
        if x_mid is None or eff_length <= 0:
            return 'Vorne'
        if front_at_x_max:
    ...
    def _cluster_positions_for_layer(items: List[Tuple[int, float]]) -> Dict[int, str]:
        """Erkennt X-Gruppen je echter Lage/Seite.

        Keine starre Drittel-Logik:
        - 1 Gruppe  -> vorne/hinten nach X-Mitte
        - 2 Gruppen -> hinten + vorne
        - 3+ Gruppen -> hinten + mitte + vorne
        """
        if not items:
            return {}

        # Einträge mit ähnlicher X-Mitte zu Gruppen zusammenfassen.
        # 600 mm ist bewusst nur eine Toleranz für fast gleiche Positionen,
        # nicht eine Drittelung der ganzen Pritsche.
        tol = 600.0
        sorted_items = sorted(items, key=lambda t: t[1])
        clusters: List[List[Tuple[int, float]]] = []
        for item in sorted_items:
            if not clusters or abs(item[1] - sum(x for _, x in clusters[-1]) / max(len(clusters[-1]), 1)) > tol:
                clusters.append([item])
            else:
                clusters[-1].append(item)

        # Wenn durch viele kleine Positionsunterschiede mehr als 3 Gruppen entstehen,
        # die nächsten Gruppen zusammenführen, bis nur noch 3 übrig sind.
        while len(clusters) > 3:
            centers = [sum(x for _, x in c) / len(c) for c in clusters]
            gaps = [centers[i + 1] - centers[i] for i in range(len(centers) - 1)]
            merge_i = min(range(len(gaps)), key=lambda i: gaps[i])
        ...
        if len(clusters) == 1:
            # BSD-Regel:
            # Ein einzelner X-Stapel / mittiger Restbund / oberer Restbund wird als VORNE geführt.
            # Nicht nach X-Koordinate auf hinten ziehen.
            for idx in order:
                label_by_cluster[idx] = 'Vorne'
        elif len(clusters) == 2:
            if front_at_x_max:
                label_by_cluster[order[0]] = 'Hinten'
                label_by_cluster[order[1]] = 'Vorne'
            else:
                label_by_cluster[order[0]] = 'Vorne'
                label_by_cluster[order[1]] = 'Hinten'
        else:
            if front_at_x_max:
                label_by_cluster[order[0]] = 'Hinten'
                label_by_cluster[order[1]] = 'Mitte'
                label_by_cluster[order[2]] = 'Vorne'
            else:
                label_by_cluster[order[0]] = 'Vorne'
                label_by_cluster[order[1]] = 'Mitte'
                label_by_cluster[order[2]] = 'Hinten'

        result: Dict[int, str] = {}
        for cluster_idx, cluster in enumerate(clusters):
            lbl = label_by_cluster.get(cluster_idx, 'Vorne')
            for row_idx, _ in cluster:
                result[row_idx] = lbl
        return result

    def _build_x_zone_lookup() -> Dict[Tuple[int, str], str]:
        """Erkennt hinten/mitte/vorne gemäss BSD-Textlogik.

        Wichtig V84:
        Mitte entsteht NICHT aus der globalen Pritschenmitte und auch NICHT aus
        einer globalen Stapelgruppierung über die ganze Seite.

        Mitte entsteht nur in einer konkreten Lage / Bundreihe:
        - pro Seite und Z-Lage werden die vorhandenen X-Gruppen angeschaut
        - 1 X-Gruppe  -> vorne oder hinten
        - 2 X-Gruppen -> hinten + vorne
        - 3 X-Gruppen -> hinten + mitte + vorne

        Dadurch bleibt z. B. ein einzelner oberer Bund vorne/rechts und wird
        nicht fälschlich Mitte rechts, nur weil seine X-Position irgendwo
        zwischen anderen Stapeln liegt.
        """
        ...
        for ridx, r in rows.iterrows():
            ...
            if typ in ['Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz']:
                continue
            x_mid = _row_x_mid(r)
            if x_mid is None:
                continue
            z_key = round(safe_number(r.get('Z_mm')), 1)
            for side in _side_names(r):
                groups.setdefault((z_key, side), []).append((ridx, x_mid))

        for (_z_key, side), items in groups.items():
            zone_by_row = _cluster_positions_for_layer(items)
            for ridx, zone in zone_by_row.items():
                lookup[(ridx, side)] = zone

        return lookup

    x_zone_lookup = _build_x_zone_lookup()

    def _position_slots_6(row: pd.Series) -> List[str]:
        slots_out: List[str] = []
        for side in _side_names(row):
            zone = x_zone_lookup.get((row.name, side))
            if not zone:
                zone = _fallback_front_back_by_x(row)
            slots_out.append(f'{zone} {side}')
        return slots_out
    ...
    # Bauteile/Bunde als einzelne sichtbare Einträge je 6er-Position zerlegen.
    for _, row in rows.sort_values(['Z_mm', 'X_mm', 'Y_mm'], kind='stable').iterrows():
        typ = str(row.get('Typ', '') or '').strip()
        if typ in ['Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz']:
            continue
        slots_for_row = _position_slots_6(row)
        ...
        # Interne Reihenfolge bleibt fachlich wie bisher: Liste oben->unten, Z unten->oben.
        ...
        for i, label in enumerate(labels_bottom_to_top):
            ...
            if i < count - 1 and internal_spacer > 0:
                # BSD V84:
                # Einlage innerhalb eines Bundes wird NICHT als eigene sichtbare Zeile ausgegeben,
                # weil sie den Bund optisch trennt. Die Höhe bleibt intern erhalten.
                z_cursor += internal_spacer

    # Kantholz ganz unten je belegter 6er-Spalte.
    if base_height > 0:
    ...
    # Einlagen zwischen separaten Lagen je 6er-Spalte.
    ...
    for z_val in layer_z_values:
        ...
        for _, lrow in layer_rows.iterrows():
            ltyp = str(lrow.get('Typ', '') or '').strip()
            if ltyp in ['Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz']:
                continue
            slots_for_lrow = _position_slots_6(lrow)
            is_bundle_layer = ltyp == 'Bund'
            ...
            for slot in slots_for_lrow:
                if (z_spacer, slot) not in existing_spacer_keys and (z_spacer, slot) not in existing_real_keys:
                    add_entry(z_spacer, slot, _fmt_bsd_mm_label('Einlage', effective_layer_spacer), spacer_kind, effective_layer_spacer, lrow['Länge_mm'], lrow['Breite_mm'], 0.0, '')

    if not entries:
        return pd.DataFrame(columns=columns)

    # Einlage-/Lagenholz-Zeilen nicht exakt auf Bauteil-Z setzen.
    ...
    # V84: geometrischer Abgleich für die BSD-Darstellung.
    # Die echte Z-Höhe bestimmt die ungefähre optische Lage.
    # Wichtig: Im PDF werden daraus KEINE gemeinsamen Tabellenlinien gezeichnet.
    # So bleiben aufliegende/obere Bunde optisch oben und werden nicht als eigener Bodenstapel
    # nach unten gezogen.
    z_levels = sorted({round(float(e.get('z', 0.0)), 1) for e in entries})
    display_levels = list(reversed(z_levels)) if top_first else z_levels
    lage_lookup = {z: i + 1 for i, z in enumerate(z_levels)}

    matrix_rows: List[Dict[str, Any]] = []
    for z_key in display_levels:
        level_entries = [e for e in entries if round(float(e.get('z', 0.0)), 1) == z_key]
        out: Dict[str, Any] = {
            'Pritsche': pname,
            'Fuhre_Nr': platform.get('Fuhre_Nr', ''),
            'Lage': lage_lookup.get(z_key, ''),
        ...
        for col in position_cols:
            out[col] = ''
            out[f'Bemerkung {col.lower()}'] = ''

        for slot in position_cols:
...
def _pdf_add_visible_spacer_rows(rows: pd.DataFrame, platform: pd.Series, view: str) -> pd.DataFrame:
    ...
    # V117 Kantholz erste Lage: nicht mehr als durchgehende Ebene über die
    # komplette Ladung zeichnen. Kantholz liegt quer auf der physischen Pritsche
    # und erzeugt im Überhang vorne/hinten keine Auflage.
    if base_height > 0:
        base_len = safe_number(platform.get('Länge_mm'), 0.0)
        back_allow = safe_number(platform.get('Überhang_hinten_mm'), 0.0)
        platform_width = safe_number(platform.get('Breite_mm'), 0.0)
        support_len = max(80.0, min(180.0, base_height * 1.6))
        px0 = back_allow
        px1 = back_allow + base_len
        if base_len > support_len and platform_width > 0:
            centers = [px0 + support_len / 2.0, (px0 + px1) / 2.0, px1 - support_len / 2.0]
            seen_centers = set()
            for cx in centers:
                key = round(cx, 1)
                ...
                kx = max(px0, min(px1 - support_len, cx - support_len / 2.0))
                helpers.append(helper_row(None, 'Kantholz', _fmt_bsd_mm_label('Kantholz', base_height), kx, 0.0, 0.0, support_len, platform_width, base_height))
...
def _pdf_draw_underbau_blocks(c, rx: float, ry: float, rw: float, rh: float, view: str, label: str) -> None:
    """V117: Unterbau/Auflager nur dezent als transparente Hilfsmarkierung.

    Wichtig: Auflager dürfen in der Stirnansicht keine Bauteile und keine
    Nummern verdecken. Deshalb wird nur gepunktet/gestrichelt im Hintergrund
    gezeichnet und auf Textlabels weitgehend verzichtet.
    """
    from reportlab.lib import colors
    if rw <= 0 or rh <= 0:
        return
    ...
    # Seiten-/Front-/Rückansichten: Auflager als zwei dezente Auflagerzonen.
    block_w = max(3.0, min(max(rh, 4.0), rw * 0.18, 28.0))
    if rw <= block_w * 2.4:
        bx = rx + (rw - block_w) / 2
...
def _pdf_dim_line_h(c, x0: float, x1: float, y: float, label: str, tick: float = 4.0) -> None:
    """Einfache horizontale Masslinie für PDF-Ansichten."""
    from reportlab.lib import colors
    if x1 < x0:
        x0, x1 = x1, x0
    if abs(x1 - x0) < 2:
    ...
    c.drawCentredString((x0 + x1) / 2.0, y + 2.0, label)
    c.restoreState()


def _pdf_dim_line_v(c, x: float, y0: float, y1: float, label: str, tick: float = 4.0) -> None:
    """Einfache vertikale Masslinie für PDF-Ansichten."""
    from reportlab.lib import colors
    if y1 < y0:
        y0, y1 = y1, y0
    if abs(y1 - y0) < 2:
    ...
    c.setLineWidth(0.35)
    c.line(x, y0, x, y1)
    c.line(x - tick, y0, x + tick, y0)
    c.line(x - tick, y1, x + tick, y1)
    c.translate(x + 6.0, (y0 + y1) / 2.0)
    c.rotate(90)
    c.setFont('Helvetica', 6.2)
...
def _pdf_real_load_rows_for_dimensions(rows: pd.DataFrame) -> pd.DataFrame:
    """Bauteile/Bunde für Bemassung, ohne Hilfsteile wie Kantholz und Einlagen."""
    if rows is None or rows.empty:
        return pd.DataFrame(columns=getattr(rows, 'columns', []))
    typ_series = rows.get('Typ', pd.Series(dtype=str)).astype(str)
    skip = ['Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz', 'Unterbau']
...
def _pdf_rects_overlap_data(a: Tuple[float, float, float, float], b: Tuple[float, float, float, float]) -> bool:
    """Prüft Überlappung zweier Rechtecke in Projektionskoordinaten."""
    ax0, ay0, ax1, ay1 = a
    bx0, by0, bx1, by1 = b
    return ax1 > bx0 and bx1 > ax0 and ay1 > by0 and by1 > ay0


def _pdf_subtract_rect_by_rects(rect: Tuple[float, float, float, float], blockers: List[Tuple[float, float, float, float]]) -> List[Tuple[float, float, float, float]]:
    """Sichtbare Restflächen eines Rechtecks nach Abzug näherer Rechtecke.

    Die Ausgabe bleibt in einfachen achsparallelen Rechteckstücken. Das reicht für
    die Stirnansicht: verdeckte Teile fallen weg, nur noch sichtbare Randstücke
    hinterer Elemente können schraffiert gezeichnet werden.
    """
    x0, y0, x1, y1 = rect
    if x1 <= x0 or y1 <= y0:
        return []
    ...
    for bx0, by0, bx1, by1 in blockers:
        ...
        if cx1 <= cx0 or cy1 <= cy0:
            continue
        relevant.append((cx0, cy0, cx1, cy1))
        xs.extend([cx0, cx1])
        ys.extend([cy0, cy1])
    if not relevant:
        return [rect]
    ...
    for i in range(len(xs) - 1):
        for j in range(len(ys) - 1):
            ...
            covered = any(bx0 <= mx <= bx1 and by0 <= my <= by1 for bx0, by0, bx1, by1 in relevant)
            if not covered:
                pieces.append((sx0, sy0, sx1, sy1))
    return pieces
...
def _v117_basis_metrics_for_platform(placements_df: pd.DataFrame, platform: pd.Series) -> Dict[str, float]:
    """V117: Kennzahl für tragende Basis der gewählten Fuhre.

    Zeigt in der selektiven Neuberechnung, ob die unterste reale Lage als
    Gesamtbasis zur darüberliegenden Lastprojektion passt. Es verändert selbst
    keine Verladung; es ist eine nachvollziehbare Kontrolle für Fuhren wie F03.
    """
    if placements_df is None or placements_df.empty or platform is None:
        return {}
    pname = str(platform.get('Pritsche', ''))
    rows = placements_df[placements_df.get('Pritsche', pd.Series(dtype=str)).astype(str).eq(pname)].copy()
    if rows.empty:
        return {}
    helper_types = {'Unterbau', 'Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz'}
    rows = rows[~rows.get('Typ', pd.Series(dtype=str)).astype(str).isin(helper_types)].copy()
    if rows.empty:
        return {}
    for col in ['X_mm', 'Y_mm', 'Z_mm', 'Länge_mm', 'Breite_mm', 'Gewicht_kg']:
        rows[col] = pd.to_numeric(rows.get(col, 0.0), errors='coerce').fillna(0.0)
    base_z = float(rows['Z_mm'].min())
    base_rows = rows[rows['Z_mm'].sub(base_z).abs() <= 2.0].copy()
    if base_rows.empty:
        return {}
    base_rects = []
    all_rects = []
    for _idx, r in base_rows.iterrows():
        x0 = safe_number(r.get('X_mm')); y0 = safe_number(r.get('Y_mm'))
        base_rects.append((x0, x0 + safe_number(r.get('Länge_mm')), y0, y0 + safe_number(r.get('Breite_mm'))))
    for _idx, r in rows.iterrows():
        x0 = safe_number(r.get('X_mm')); y0 = safe_number(r.get('Y_mm'))
        all_rects.append((x0, x0 + safe_number(r.get('Länge_mm')), y0, y0 + safe_number(r.get('Breite_mm'))))
    base_area = _rect_union_area(base_rects)
    all_area = _rect_union_area(all_rects)
    upper_weight = float(rows.loc[rows['Z_mm'] > base_z + 2.0, 'Gewicht_kg'].sum()) if 'Gewicht_kg' in rows.columns else 0.0
    total_weight = float(rows['Gewicht_kg'].sum()) if 'Gewicht_kg' in rows.columns else 0.0
    return {
        'Basis_Bauteile': float(len(base_rows)),
        'Basisfläche_m2': base_area / 1_000_000.0,
        'Lastprojektion_m2': all_area / 1_000_000.0,
        'Basis_zu_Lastprojektion_%': 100.0 * base_area / all_area if all_area > 0 else 0.0,
        'Gewicht_oberhalb_kg': upper_weight,
        'Gesamtgewicht_kg': total_weight,
    }
...
def _pdf_draw_view(c, placements: pd.DataFrame, platform: pd.Series, x: float, y: float, w: float, h: float, view: str, title: str, front_at_x_max: bool = False, left_at_y_max: bool = False, bundle_overview_only: bool = False, show_dimensions: bool = True, scale_context: Optional[Dict[str, float]] = None) -> None:
    ...
    over_front_allowed = safe_number(platform.get('Überhang_vorne_mm'))
    over_back_allowed = safe_number(platform.get('Überhang_hinten_mm'))
    eff_length = base_length + over_front_allowed + over_back_allowed
    platform_x0 = over_back_allowed
    platform_x1 = over_back_allowed + base_length
    width = safe_number(platform.get('Breite_mm'))
    ...
    rows = rows[rows['X_mm'].notna() & rows['Y_mm'].notna() & rows['Z_mm'].notna()].copy()
    dim_rows = _pdf_real_load_rows_for_dimensions(rows)
    ...
    if not dim_rows.empty:
        ...
        load_y0 = safe_number(dim_rows['Y_mm'].min(), 0.0)
        load_y1 = safe_number((dim_rows['Y_mm'] + dim_rows['Breite_mm']).max(), 0.0)
        used_len = max(load_x1 - load_x0, 0.0)
        used_wid = max(load_y1 - load_y0, 0.0)
        used_hei = safe_number((dim_rows['Z_mm'] + dim_rows['Höhe_mm']).max(), 0.0)
    else:
    ...
    if view == 'top':
        ...
        c.setStrokeColor(colors.black)
        c.setLineWidth(0.45)
        c.rect(ox, oy - bar_h, draw_w, bar_h, stroke=1, fill=1)
    ...
    def draw_projected_fragment(row: pd.Series, px: float, py: float, pw: float, ph: float, ghost: bool = False, draw_label: bool = True) -> None:
        rx = ox + (px - view_x_offset) * scale_x
        ry = oy + (py - view_y_offset) * scale_y
        rw = pw * scale_x
        rh = ph * scale_y
        if rw <= 0 or rh <= 0:
            return
    ...
    # V117: Hilfselemente (Kantholz, Einlagen, Unterbau/Auflager) immer zuerst
    # und im Hintergrund zeichnen. Sie dürfen in Stirn- und Seitenansicht keine
    # Bauteile überdecken und zählen auch nicht als Sichtblocker für die
    # Schraffurlogik.
    helper_draw_types_v117 = {'Kantholz', 'Bundeinlage', 'Einlage', 'Lagenholz', 'Unterbau'}
    if rows is not None and not rows.empty:
        typ_series_v117 = rows.get('Typ', pd.Series(dtype=str)).astype(str)
        helper_bg_rows_v117 = rows[typ_series_v117.isin(helper_draw_types_v117)].copy()
        rows = rows[~typ_series_v117.isin(helper_draw_types_v117)].copy()
        if not helper_bg_rows_v117.empty:
            helper_bg_rows_v117['_pdf_sort'] = helper_bg_rows_v117.apply(lambda r: _pdf_visible_sort_value(r, view, eff_length, width, front_at_x_max, left_at_y_max), axis=1)
            helper_bg_rows_v117 = helper_bg_rows_v117.sort_values(['Z_mm', '_pdf_sort', 'X_mm', 'Y_mm'], kind='stable')
            for _, helper_row_v117 in helper_bg_rows_v117.iterrows():
                draw_projected_row(helper_row_v117, ghost=False)
    ...
    if view in ('front', 'back') and rows is not None and not rows.empty:
        ...
        for _, row in rows_front.iterrows():
            ...
            if pw <= 0 or ph <= 0:
                continue
            row_typ = str(row.get('Typ', '') or '').strip()
            has_front_overlap = any(_pdf_rects_overlap_data(rect, b) for b in blockers)
            pieces = _pdf_subtract_rect_by_rects(rect, blockers)
            if not pieces:
                continue
...
def _bsd_cell_text_for_matrix(value: Any) -> str:
    ...
    Wichtig:
    - Einlage bleibt sichtbar, weil die gewünschte TXT-Matrix Einlage-Zeilen enthält.
    - Mehrere Einträge in einer Zelle werden kompakt mit / getrennt.
    """
    txt = str(value or '').strip()
    if not txt:
        return ''
...
def _bsd_label_bundle_prefix(value: str) -> str:
    """Ermittelt Bxx aus einem BSD-Label wie B24:1802."""
    txt = str(value or '').strip()
    m = re.match(r'^(B\d+):', txt)
    return m.group(1) if m else ''


def _bsd_text_matrix_rows(matrix_df: pd.DataFrame) -> Tuple[List[str], List[List[str]]]:
    """Echte 6-Spalten-Zeilenmatrix für BSD.

    Keine Spaltenverdichtung.
    Keine Stapelkästen.
    ...
    for _, row in matrix_df.iterrows():
        vals = [_bsd_cell_text_for_matrix(row.get(col, '')) for col in position_cols]
        if any(v.strip() for v in vals):
            raw_rows.append(vals)

    # Schutz gegen optische Bund-Trennung:
    # Einlage in Spalte X fällt weg, wenn direkt davor und danach dieselbe Bundnummer steht.
    cleaned: List[List[str]] = []
    for i, vals in enumerate(raw_rows):
        new_vals = list(vals)
        for ci, value in enumerate(vals):
            low = str(value or '').lower().strip()
            if not low.startswith('einlage'):
                continue
            prev_prefix = ''
            next_prefix = ''
            j = i - 1
            while j >= 0 and not prev_prefix:
                prev_prefix = _bsd_label_bundle_prefix(raw_rows[j][ci])
                if str(raw_rows[j][ci] or '').lower().startswith('einlage'):
                    break
                j -= 1
            j = i + 1
            while j < len(raw_rows) and not next_prefix:
                next_prefix = _bsd_label_bundle_prefix(raw_rows[j][ci])
                if str(raw_rows[j][ci] or '').lower().startswith('einlage'):
                    break
                j += 1
            if prev_prefix and prev_prefix == next_prefix:
                new_vals[ci] = ''
        if any(v.strip() for v in new_vals):
            cleaned.append(new_vals)

    return position_cols, cleaned


def _bsd_excel_inline_einlage_in_previous_cell(matrix_rows: List[List[str]]) -> List[List[str]]:
    """Nur Excel-BSD: Einlage in den vorherigen Bundblock derselben Spalte schreiben.

    Dadurch entsteht keine eigene Tabellenzeile durch "Einlage 40".
    Andere Logik / Positionen bleiben unverändert.
    """
    out: List[List[str]] = []
    for vals in matrix_rows:
        new_vals = list(vals)
        for ci, value in enumerate(vals):
            txt = str(value or '').strip()
            if not txt.lower().startswith('einlage'):
                continue

            target_ri = None
            for ri in range(len(out) - 1, -1, -1):
                prev = str(out[ri][ci] or '').strip()
                if prev and not prev.lower().startswith('einlage'):
                    target_ri = ri
                    break

            if target_ri is not None:
                if txt not in str(out[target_ri][ci]):
                    out[target_ri][ci] = str(out[target_ri][ci]).rstrip() + '\n' + txt
                new_vals[ci] = ''

        if any(str(v or '').strip() for v in new_vals):
            out.append(new_vals)

    return out


def _bsd_excel_variant_b_rows(matrix_rows: List[List[str]]) -> List[List[str]]:
    """Nur Excel-BSD Variante B.

    Einlage bleibt beim Bundblock, aber als kleine Zusatzzeile direkt darunter.
    Keine Rich-Text-Zelle, dadurch Excel-stabil.
    """
    inlined = _bsd_excel_inline_einlage_in_previous_cell(matrix_rows)
    out: List[List[str]] = []

    for vals in inlined:
        base_vals = ['' for _ in vals]
        einlage_vals = ['' for _ in vals]
        ...
        for ci, value in enumerate(vals):
            ...
            base_lines: List[str] = []
            einlage_lines: List[str] = []
            for line in txt.split('\n'):
                line_txt = str(line or '').strip()
                if not line_txt:
                    continue
                if line_txt.lower().startswith('einlage'):
                    einlage_lines.append(line_txt)
                else:
                    base_lines.append(line_txt)

            base_vals[ci] = '\n'.join(base_lines)
            einlage_vals[ci] = '\n'.join(einlage_lines)

        if any(v.strip() for v in base_vals):
            out.append(base_vals)
        if any(v.strip() for v in einlage_vals):
            out.append(einlage_vals)

    return out


def _bsd_excel_is_einlage_row(row_values: List[str]) -> bool:
    non_empty = [str(v or '').strip() for v in row_values if str(v or '').strip()]
    return bool(non_empty) and all(v.lower().startswith('einlage') for v in non_empty)


def _bsd_excel_is_bare_part_row(row_values: List[str]) -> bool:
    """Nur Excel-BSD: Zeile enthält nur einzelne Bauteilnummern ohne Bundprefix."""
    non_empty = [str(v or '').strip() for v in row_values if str(v or '').strip()]
    if not non_empty:
        return False
    for v in non_empty:
        # keine Bundzeile, keine Einlage, kein Kantholz
        if v.lower().startswith(('einlage', 'kantholz')):
            return False
        if ':' in v:
            return False
...
def _bsd_excel_pair_adjacent_bare_part_rows(matrix_rows: List[List[str]]) -> List[List[str]]:
    """Nur Excel-BSD.

    Einzelbauteile ohne Bundprefix, die links/rechts alternierend auf zwei
    aufeinanderfolgenden Zeilen stehen, werden optisch in eine Zeile gezogen.
    Das reduziert die Luft zwischen den einzelnen Bauteilen, ohne Bundlogik,
    Mitte-Logik oder Einlage-Logik zu ändern.
    """
    out: List[List[str]] = []
    i = 0
    while i < len(matrix_rows):
        current = list(matrix_rows[i])
        if i + 1 < len(matrix_rows):
            nxt = list(matrix_rows[i + 1])
            if _bsd_excel_is_bare_part_row(current) and _bsd_excel_is_bare_part_row(nxt):
                conflict = any(str(current[c] or '').strip() and str(nxt[c] or '').strip() for c in range(min(len(current), len(nxt))))
                if not conflict:
                    merged = [str(current[c] or '').strip() or str(nxt[c] or '').strip() for c in range(len(current))]
                    out.append(merged)
                    i += 2
                    continue
        out.append(current)
...
def _bsd_excel_has_bare_part(row_values: List[str]) -> bool:
    """Nur Excel-BSD: Zeile enthält mindestens ein Einzelbauteil ohne Bundprefix."""
    for v in row_values:
        txt = str(v or '').strip()
        if not txt:
            continue
        if txt.lower().startswith(('einlage', 'kantholz')):
            continue
        if ':' in txt:
            continue
...
def _bsd_excel_compact_leading_bund_columns(matrix_rows: List[List[str]]) -> List[List[str]]:
    """Nur Excel-BSD, minimaler Eingriff.

    Führende Bundblöcke vor den ersten Einzelbauteilen werden spaltenweise
    nach oben verdichtet. Dadurch liegen z. B. B71 / B72 / B73 oben bündig,
    ohne spätere Bereiche oder die Verladelogik zu verändern.
    """
    if not matrix_rows:
        return matrix_rows

    first_bare_idx = None
    for idx, vals in enumerate(matrix_rows):
        if _bsd_excel_has_bare_part(vals):
            first_bare_idx = idx
            break

    # Ohne Einzelbauteil-Grenze nichts ändern: minimal bleiben.
    if first_bare_idx is None or first_bare_idx <= 0:
        return matrix_rows

    lead = [list(row) for row in matrix_rows[:first_bare_idx]]
    rest = [list(row) for row in matrix_rows[first_bare_idx:]]
    col_count = len(lead[0]) if lead else 0

    # Nur anwenden, wenn im führenden Bereich Bundprefixe vorkommen.
    if not any(':' in str(v or '') for row in lead for v in row):
        return matrix_rows

    columns: List[List[str]] = []
    for ci in range(col_count):
        col_vals = [str(row[ci] or '').strip() for row in lead if str(row[ci] or '').strip()]
        columns.append(col_vals)

    new_len = max((len(c) for c in columns), default=0)
    if new_len <= 0:
        return matrix_rows

    compacted = [['' for _ in range(col_count)] for _ in range(new_len)]
    for ci, col_vals in enumerate(columns):
        for ri, value in enumerate(col_vals):
            compacted[ri][ci] = value

    return compacted + rest


def _bsd_excel_is_stop_row(row_values: List[str]) -> bool:
    """Nur BSD-Tabelle: Einlage/Kantholz-Zeilen nicht mit anderen Zeilen mischen."""
    for v in row_values:
        txt = str(v or '').strip().lower()
        if txt.startswith(('einlage', 'kantholz')):
            return True
    return False


def _bsd_excel_merge_safe_row_pairs(matrix_rows: List[List[str]]) -> List[List[str]]:
    """Nur BSD-Tabelle, minimaler Eingriff.

    Nebeneinanderliegende Zeilen werden nur dann zusammengezogen, wenn keine
    Spalte kollidiert. Einlage/Kantholz bleibt als eigene Zusatzzeile direkt
    beim zugehörigen Bundblock und wird nicht mit Bauteilen vermischt.
    """
    ...
    while changed:
        ...
        while i < len(rows):
            ...
            if i + 1 < len(rows):
                nxt = list(rows[i + 1])
                if not _bsd_excel_is_stop_row(current) and not _bsd_excel_is_stop_row(nxt):
                    max_len = min(len(current), len(nxt))
                    conflict = any(str(current[c] or '').strip() and str(nxt[c] or '').strip() for c in range(max_len))
                    if not conflict:
                        merged = [str(current[c] or '').strip() or str(nxt[c] or '').strip() for c in range(len(current))]
                        out.append(merged)
                        i += 2
...
def _bsd_display_matrix_rows(matrix_df: pd.DataFrame) -> Tuple[List[str], List[List[str]]]:
    """Einheitliche BSD-Tabellenzeilen für Excel/TXT/PDF.

    V84:
    Der zusätzliche Bundblock-Verdichter aus v74/v75 wird NICHT mehr pauschal
    auf alle Pritschen angewendet. Er war für F02 Anhänger / B71-B73 nötig,
    hat bei F01 LKW aber den Bund Mitte links optisch nach oben gezogen.

    Minimaler Eingriff:
    - Standard bleibt wie v73: Einlage bleibt beim Bund, Einzelteile kompakter.
    - Der spezielle obere-Bund-Verdichter läuft nur noch, wenn die bekannte
      F02-Anhänger-Konstellation B71/B72/B73 vorhanden ist.
    - Keine Änderung an Verladung / Geometrie / Packlogik.
    """
    position_cols, raw_rows = _bsd_text_matrix_rows(matrix_df)
    rows = _bsd_excel_variant_b_rows(raw_rows)
    rows = _bsd_excel_pair_adjacent_bare_part_rows(rows)

    flat_labels = ' '.join(str(v or '') for row in rows for v in row)
    is_f02_anhaenger_top_bundle_case = all(token in flat_labels for token in ('B71:', 'B72:', 'B73:'))

    if is_f02_anhaenger_top_bundle_case:
        rows = _bsd_excel_compact_leading_bund_columns(rows)
        rows = _bsd_excel_merge_safe_row_pairs(rows)

    return position_cols, rows


def format_bsd_simple_text_matrix(matrix_df: pd.DataFrame) -> str:
    """Erzeugt die BSD als echte 6-Spalten-TXT-Matrix."""
    position_cols, matrix_rows = _bsd_display_matrix_rows(matrix_df)
    if not matrix_rows:
        return ''

    col_width = 18
    lines: List[str] = []
    header = ''.join(col.ljust(col_width) for col in position_cols)
    lines.append(header.rstrip())
    lines.append('-' * min(len(header.rstrip()), col_width * len(position_cols)))

    for vals in matrix_rows:
        line_parts: List[str] = []
        for value in vals:
            line_parts.append(str(value)[:col_width - 1].ljust(col_width))
...
def _pdf_draw_bsd_matrix_page(c, page_w: float, page_h: float, margin: float, platform: pd.Series, matrix_df: pd.DataFrame, header: Dict[str, Any], project_name: str, logo_bytes: Optional[bytes] = None) -> None:
    """Zeichnet eine einfache BSD-Seite als Textmatrix.

    V84:
    - keine Boxen je Eintrag
    - keine Einlage als eigene sichtbare Box/Zeile
    - keine gemeinsamen waagerechten Linien
    - BSD bleibt Leseliste; Geometrie bleibt im Lageplan
    """
    from reportlab.lib import colors
    ...
    if rows.empty:
        c.setFont('Helvetica', 8)
        c.drawString(margin, top_y - 160, 'Keine Ladeplan-BSD-Daten vorhanden')
        return

    # Einheitliche BSD-Tabelle wie Excel:
    # Bundblock + zugehörige Einlage bleiben zusammen.
    position_cols, matrix_rows = _bsd_display_matrix_rows(rows)
    if not matrix_rows:
        c.setFont('Helvetica', 8)
        c.drawString(margin, top_y - 160, 'Keine sichtbaren BSD-Einträge vorhanden')
        return
    ...
    area_bottom = 42
    gap = 6
    col_w = (page_w - 2 * margin - gap * (len(position_cols) - 1)) / len(position_cols)
    header_h = 20
    usable_h = max(80, area_top - area_bottom - header_h - 8)
    row_h = min(15.0, max(7.5, usable_h / max(len(matrix_rows), 1)))
    font_size = 7.0 if row_h >= 10 else 5.8

    # Spaltenkopf. Keine Stapelkästen; nur einfache Textmatrix mit leichten Spaltentrennern.
    for ci, col in enumerate(position_cols):
        x0 = area_x + ci * (col_w + gap)
        c.setFillColor(colors.lightgrey)
        c.setStrokeColor(colors.black)
        c.setLineWidth(0.45)
        c.rect(x0, area_top - header_h, col_w, header_h, stroke=1, fill=1)
        c.setFillColor(colors.black)
        c.setFont('Helvetica-Bold', 7.0)
        c.drawCentredString(x0 + col_w / 2.0, area_top - 13, col)
        c.setStrokeColor(colors.lightgrey)
        c.setLineWidth(0.25)
        c.line(x0, area_bottom, x0, area_top - header_h)
        c.line(x0 + col_w, area_bottom, x0 + col_w, area_top - header_h)

    def _draw_cell_text(x0: float, y0: float, w0: float, txt: str) -> None:
        txt = str(txt or '').strip()
        if not txt:
            return
        c.setFont('Helvetica', font_size)
        max_chars = max(8, int(w0 / (font_size * 0.52)))
        if len(txt) > max_chars:
            txt = txt[:max_chars - 1] + '…'
        c.drawString(x0 + 3, y0, txt)

    y = area_top - header_h - row_h + 3.5
    for vals in matrix_rows:
        if y < area_bottom:
            break
        for ci, value in enumerate(vals):
            x0 = area_x + ci * (col_w + gap)
            _draw_cell_text(x0, y, col_w - 6, value)
        y -= row_h
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    # Dokument-Logo nur verwenden, wenn explizit hochgeladen.
    # Das eingebaute Logo bleibt nur App-Logo in der Seitenleiste.
    logo_bytes = None
    ...
    second_attr = col3.selectbox('Nebenattribut 1', ['Keine'] + sort_options, index=0)
    second_direction = col4.selectbox('Richtung Nebenattribut 1', ['aufsteigend', 'absteigend'], index=0)

    extra_sort_attrs: List[Tuple[str, bool]] = []
    with st.expander('Weitere Nebenattribute für Sortierung', expanded=False):
        st.caption('Die Sortierung läuft der Reihe nach: Hauptattribut → Nebenattribut 1 → Nebenattribut 2 → Nebenattribut 3 → Nebenattribut 4.')
        for sort_idx in range(2, 5):
            scol1, scol2 = st.columns(2)
            attr = scol1.selectbox(
                f'Nebenattribut {sort_idx}',
                ['Keine'] + sort_options,
                index=0,
                key=f'sort_extra_attr_{sort_idx}_v84',
            )
            direction = scol2.selectbox(
                f'Richtung Nebenattribut {sort_idx}',
                ['aufsteigend', 'absteigend'],
                index=0,
                key=f'sort_extra_dir_{sort_idx}_v84',
            )
            if attr != 'Keine':
                extra_sort_attrs.append((attr, direction == 'aufsteigend'))
    ...
    sorted_parts = sort_parts_dataframe(
        ...
        extra_sort_attrs=extra_sort_attrs,
    )
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v84')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v84') if option_choices_manual else ''
    ...
    if not bool(st.session_state.get('loading_plan_started_v84', False)):
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v84', False)) if verladeart == 'Manuell' else True
            ...
            if verladeart != 'Automatisch':
                ...
                if p_match.empty:
                    ...
                    if target_units.empty:
                        ...
                        recalc_stabilize_basis = bc1.checkbox(
                            'tragende Basis / unterste Lage stabilisieren',
                            value=False,
                            key='v117_recalc_stabilize_basis',
                            help='Nur für die gewählte Fuhre. Versucht eine ruhigere Basis: hintereinander statt nebeneinander bevorzugen und grosse/schwere Teile eher unten halten.'
                        )
                        recalc_basis_report = bc2.checkbox(
                            'Basisprüfung in Vorschau anzeigen',
                            value=True,
                            key='v117_recalc_basis_report',
                        )
                        if recalc_stabilize_basis:
                            st.caption('V117: Für diese Vorschau wird nebeneinander auf der Basis reduziert und hintereinander bevorzugt. Andere Fuhren bleiben unverändert.')
                        ...
                        def _v117_basis_order_units(local_units: pd.DataFrame) -> pd.DataFrame:
                            ...
                            # Grössere/schwerere Einheiten werden bei aktivierter Basis-Stabilisierung
                            # früher versucht, damit sie eher unten und ruhiger verteilt liegen.
                            if local_units is None or local_units.empty:
                                return local_units
                            ordered = local_units.copy()
                            ordered['_V117_Footprint'] = pd.to_numeric(ordered.get('Länge_mm', 0.0), errors='coerce').fillna(0.0) * pd.to_numeric(ordered.get('Breite_mm', 0.0), errors='coerce').fillna(0.0)
                            ordered['_V117_Weight'] = pd.to_numeric(ordered.get('Gewicht_kg', 0.0), errors='coerce').fillna(0.0)
                            ordered['_V117_Length'] = pd.to_numeric(ordered.get('Länge_mm', 0.0), errors='coerce').fillna(0.0)
                        ...
                        def _v113_preview_for_units(local_units: pd.DataFrame, local_platform: pd.Series, suffix: str = '') -> Tuple[pd.DataFrame, pd.DataFrame]:
                            ...
                            return create_loading_plan(
                                ...
                                prefer_length_before_stack=(True if bool(recalc_stabilize_basis) else bool(recalc_prefer_length)),
                            )
                        ...
                        if isinstance(preview, dict) and preview.get('platform') == str(recalc_platform):
                            ...
                            if recalc_basis_report and not loaded_preview.empty:
                                basis_metrics = _v117_basis_metrics_for_platform(loaded_preview, preview_platform_row)
                                if basis_metrics:
                                    st.caption(
                                        f"Tragende Basis: {basis_metrics.get('Basis_Bauteile', 0):.0f} Teile, "
                                        f"Basisfläche {basis_metrics.get('Basisfläche_m2', 0):.2f} m², "
                                        f"Basis zu Lastprojektion {basis_metrics.get('Basis_zu_Lastprojektion_%', 0):.0f} %, "
                                        f"Gewicht oberhalb ca. {basis_metrics.get('Gewicht_oberhalb_kg', 0):.0f} kg."
                                    )
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v84')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v84'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v84'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v84'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v84', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v84', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v84')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v84')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v84',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v84', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v84', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v84',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v84')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v84', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v84', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v84', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v84'] = validation_df
                            st.session_state['manual_plan_ready_v84'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v84'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v84'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v84', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v84',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v84')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v84', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v84_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v84', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Echte 6-Spalten-BSD-Matrix: Bundteile bleiben zusammen; Einlage trennt keinen Bundblock.')
...
def main():
    ...
    # V84:
    # App-Logo fix sichtbar oben in der Hauptseite.
    # Das ist NICHT das optionale Dokument-Logo für PDF/Excel.
    # Anzeige über HTML-data-uri, damit Streamlit das Bild nicht still verschluckt.
    app_logo_b64 = str(DEFAULT_LOGO_B64 or '').strip()
    if app_logo_b64:
        st.markdown(
            f'''
            <div class="app-logo-wrap">
                <img src="data:image/jpeg;base64,{app_logo_b64}"
                     style="width:180px; max-width:35vw; height:auto; object-fit:contain; display:block;">
            </div>
            ''',
            unsafe_allow_html=True
        )
    ...
    with st.sidebar:
        ...
        if module == 'BVX Auswertung':
            ...
            logo_file = st.file_uploader(
                'Logo für Dokumente hochladen (optional)',
                type=['png', 'jpg', 'jpeg'],
                key='logo_upload',
                help='Optionales Dokument-Logo für PDF/Excel. Das eingebaute App-Logo steht nur oben in der App.'
            )
            analysis_file = None
            if loading_file:
