...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v40')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v40') if option_choices_manual else ''
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v40')
                default_zone_label = top2.radio(
                    'Zielzone',
                    ['vorne links', 'vorne rechts', 'hinten links', 'hinten rechts'],
                    horizontal=True,
                    key='manual_zone_target_v40',
                )
                if top3.button('Reset', key='manual_zone_reset_v40'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v40')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v40')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v40',
                        )

                        z1, z2 = st.columns(2)
                        if z1.button(f'→ {default_zone_label}', key='manual_zone_apply_default_v40', type='primary'):
                        ...
                        if z2.button('entladen', key='manual_zone_unload_v40'):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v40'):
                                manual_df = _manual_assign_unit_to_zone(
                                    manual_df,
                                    platforms_used_df,
                                    selected_unit_id,
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v40',
                        )
                ...
                with c_right:
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        manual_df = _manual_reflow_all_zones(manual_df, platforms_used_df)
                        ...
                        st.plotly_chart(
                            ...
                            use_container_width=True,
                            key=f'manual_zone_plot_v40_{selected_manual_platform}_{view_map[view_choice]}',
                        )

                # Nach manueller Eingabe alles aus dem Zonenplan berechnen.
                edited_placements_df = _manual_reflow_all_zones(st.session_state.get('manual_placements_df', manual_df), platforms_used_df)
