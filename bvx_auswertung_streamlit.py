...
def create_loading_excel(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        def set_bsd_matrix_cell(cell, value: Any):
            """Nur Excel-BSD: Bauteile gross, Einlageholz klein in derselben Zelle."""
            ...
            has_einlage = any(str(line).strip().lower().startswith('einlage') for line in lines)

            if has_einlage and len(lines) > 1:
                try:
                    ...
                    for li, line in enumerate(lines):
                        if li > 0:
                            rt.append('\n')
                        line_txt = str(line).strip()
                        is_einlage = line_txt.lower().startswith('einlage')
                        rt.append(TextBlock(
                            InlineFont(
                                rFont='Arial',
                                sz=8 if is_einlage else 16,
                                b=False if is_einlage else True,
                            ),
                            line_txt
                        ))
                    cell.value = rt
            ...
            cell.value = txt
            is_einlage_only = txt.lower().startswith('einlage')
            cell.font = Font(name='Arial', size=8 if is_einlage_only else 16, bold=not is_einlage_only)
        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            if _bsd_rows:
                for i, row_values in enumerate(_bsd_rows, start=write_start):
                    ...
                    ws.row_dimensions[i].height = 42 if multiline else 28
            ...
            # Matrix-Bauteile bleiben 16 pt, Einlage in Matrix bleibt 8 pt.
            try:
...
def create_bsd_matrix_for_platform(
    ...
    # V69: geometrischer Abgleich für die BSD-Darstellung.
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v69')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v69') if option_choices_manual else ''
    ...
    if start_col1.button('Verladung starten / neu berechnen', type='primary', key='start_loading_plan_v69'):
        st.session_state['loading_plan_started_v69'] = True
    ...
    if start_col2.button('Verladung zurücksetzen', key='reset_loading_plan_v69'):
        st.session_state['loading_plan_started_v69'] = False
        for k in ['manual_plan_signature', 'manual_platform_signature', 'manual_placements_df', 'manual_platforms_df']:
            st.session_state.pop(k, None)
        st.rerun()

    if not bool(st.session_state.get('loading_plan_started_v69', False)):
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v69', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v69')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v69'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v69'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v69'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v69', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v69', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v69')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v69')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v69',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v69', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v69', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v69',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v69')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v69', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v69', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v69', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v69'] = validation_df
                            st.session_state['manual_plan_ready_v69'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v69'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v69'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v69', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v69',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v69')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v69', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v69_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v69', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
