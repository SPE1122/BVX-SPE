        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            for r in range(data_row, data_row + max_rows):
                for c in range(1, 6):
                    ...
                    ws.cell(r, c).fill = fill_yellow if c <= 5 else fill_white
...
def create_bsd_matrix_for_platform(
    ...
    """Erstellt eine kompakte Ladeplan-BSD-Liste je Pritsche.

    V48:
    - Die BSD-Tabelle wird nicht mehr über eine gemeinsame Z-Höhe synchronisiert.
    - Jede Zone bekommt ihre eigene Stapelliste von oben nach unten.
    - Leere Zwischenzellen entstehen dadurch nur noch, wenn eine Zone effektiv weniger Einträge hat.
    - Die echte Verladeposition bleibt unverändert; nur die tabellarische Darstellung wird kompakter.
    """
    columns = [
    ...
    zone_lists: Dict[str, List[Dict[str, Any]]] = {slot: [] for slot in slots}

    def _add_zone_item(slot: str, label: str, sort_z: float, kind: str = 'Bauteil', weight: float = 0.0) -> None:
        if slot not in zone_lists:
            return
        txt = str(label or '').strip()
        if not txt:
            return
        zone_lists[slot].append({
            'label': txt,
            'sort_z': float(sort_z),
            'kind': str(kind or ''),
    ...
    # Eine Zone = eigene Stapelliste.
    # Sortierung: oben nach unten; innerhalb gleicher Höhe stabil nach X/Y.
    sorted_rows = rows.sort_values(['Z_mm', 'X_mm', 'Y_mm'], ascending=[False, True, True], kind='stable')
    for _, row in sorted_rows.iterrows():
        slots_for_row = _position_slots_for_bsd(row, eff_length, platform_width, front_at_x_max, left_at_y_max)
        if not slots_for_row:
            slots_for_row = [_position_slot_for_bsd(row, eff_length, platform_width, front_at_x_max, left_at_y_max)]
        ...
        row_top_z = safe_number(row.get('Z_mm')) + safe_number(row.get('Höhe_mm'))
        row_weight = safe_number(row.get('Gewicht_kg'))

        if typ == 'Unterbau':
            # Unterbau im BSD bewusst nicht als normale Bauteilnummer mischen.
            # Er bleibt in Visualisierung/Warnung sichtbar.
            continue

        if typ == 'Bund' and count > 1:
            labels = (labels + [labels[-1]] * count)[:count]
            # Ansicht_Liste ist logisch oben -> unten. Genau so wird sie im BSD angezeigt.
            internal_spacer = safe_number(row.get('Einlage_allgemein_mm'), platform_general_spacer)
            if internal_spacer <= 0:
                internal_spacer = 0.0
            part_weight = row_weight / count if count else row_weight
            for slot in slots_for_row:
                for i, label in enumerate(labels):
                    _add_zone_item(slot, label, row_top_z - i * 0.01, 'Bund-Bauteil', part_weight)
                    if i < count - 1 and internal_spacer > 0:
                        _add_zone_item(slot, _fmt_bsd_mm_label('Einlage', internal_spacer), row_top_z - i * 0.01 - 0.005, 'Einlage', 0.0)
        else:
            label = labels[0] if labels else _format_bsd_cell(row)
            for slot in slots_for_row:
                _add_zone_item(slot, label, row_top_z, 'Bauteil', row_weight)

        # Unterlage zwischen separaten Lagen/Bunden gehört direkt unter diese Lage.
        if safe_number(row.get('Z_mm')) > base_height + 0.1:
            if typ == 'Bund' and spacer_height > 0:
                spacer_label = _fmt_bsd_mm_label('Einlage', spacer_height)
            elif platform_general_spacer > 0:
                spacer_label = _fmt_bsd_mm_label('Einlage', platform_general_spacer)
            else:
                spacer_label = ''
            if spacer_label:
                for slot in slots_for_row:
                    _add_zone_item(slot, spacer_label, safe_number(row.get('Z_mm')) - 0.01, 'Lagenholz', 0.0)

    # Kantholz ganz unten pro belegter Zone.
    if base_height > 0:
        for slot in slots:
            if zone_lists[slot]:
                _add_zone_item(slot, _fmt_bsd_mm_label('Kantholz', base_height), -1.0, 'Kantholz erste Lage', 0.0)

    # Reihenfolge pro Zone: oben nach unten.
    for slot in slots:
        zone_lists[slot] = sorted(zone_lists[slot], key=lambda e: e.get('sort_z', 0.0), reverse=True)

    max_len = max((len(v) for v in zone_lists.values()), default=0)
    if max_len <= 0:
        return pd.DataFrame(columns=columns)
    ...
    for i in range(max_len):
        out: Dict[str, Any] = {
            'Pritsche': pname,
            'Fuhre_Nr': platform.get('Fuhre_Nr', ''),
            # Lage bleibt als Lesereihenfolge erhalten: oben grosse Nummer, unten 1.
            'Lage': max_len - i if top_first else i + 1,
            'Z_mm': '',
            'Vorne links': '',
            'Vorne rechts': '',
            'Hinten links': '',
            ...
            'Gewicht_kg': '',
            'Anzahl_Einheiten': 0,
            'Zeilentyp': 'Kompakte Zonenliste',
        }
        unit_count = 0
        weight_sum = 0.0
        for slot in slots:
            if i < len(zone_lists[slot]):
                item = zone_lists[slot][i]
                out[slot] = item.get('label', '')
                if 'Bauteil' in str(item.get('kind', '')):
                    unit_count += 1
                    weight_sum += safe_number(item.get('weight'), 0.0)
        out['Anzahl_Einheiten'] = unit_count
        out['Gewicht_kg'] = round(weight_sum, 2) if weight_sum else ''
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v48')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v48') if option_choices_manual else ''
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v48', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v48')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v48'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v48'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v48'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v48', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v48', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v48')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v48')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v48',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v48', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v48', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v48',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v48')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v48', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v48', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v48', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v48'] = validation_df
                            st.session_state['manual_plan_ready_v48'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v48'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v48'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v48', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v48',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v48')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v48', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v48_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v48', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Die Tabelle wird automatisch für jede belegte Pritsche erstellt. V48: kompakte BSD-Zonenliste. Jede Zone wird von oben nach unten gelistet; die Verladeposition selbst bleibt unverändert.')
