...
def create_loading_excel(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            # V82: BSD-Matrix A:F breiter für bessere Lesbarkeit auf Ausdruck.
            widths = {
...
def create_bsd_matrix_for_platform(
    ...
    # V82: geometrischer Abgleich für die BSD-Darstellung.
...
def _pdf_draw_view(c, placements: pd.DataFrame, platform: pd.Series, x: float, y: float, w: float, h: float, view: str, title: str, front_at_x_max: bool = False, left_at_y_max: bool = False) -> None:
    ...
    # V82: Titel "Draufsicht" etwas tiefer setzen,
    ...
    # V82: Gegenseite in Seitenansichten zuerst hellgrau im Hintergrund zeichnen.
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v82')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v82') if option_choices_manual else ''
    ...
    if start_col1.button('Verladung starten / neu berechnen', type='primary', key='start_loading_plan_v82'):
        st.session_state['loading_plan_started_v82'] = True
    ...
    if start_col2.button('Verladung zurücksetzen', key='reset_loading_plan_v82'):
        st.session_state['loading_plan_started_v82'] = False
        for k in ['manual_plan_signature', 'manual_platform_signature', 'manual_placements_df', 'manual_platforms_df']:
            st.session_state.pop(k, None)
        st.rerun()

    if not bool(st.session_state.get('loading_plan_started_v82', False)):
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v82', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v82')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v82'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v82'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v82'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v82', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v82', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v82')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v82')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v82',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v82', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v82', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v82',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v82')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v82', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v82', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v82', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v82'] = validation_df
                            st.session_state['manual_plan_ready_v82'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v82'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v82'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v82', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v82',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v82')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v82', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v82_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v82', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
