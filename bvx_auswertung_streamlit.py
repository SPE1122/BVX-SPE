...
def create_bsd_matrix_for_platform(
    ...
    - Ein Bund / eine Verladeeinheit bleibt eine BSD-Einheit.
    - Bunde werden NICHT mehr in einzelne Bauteile als künstliche Lagen zerlegt.
    - Eine echte Z-Lage bleibt eine Tabellenzeile.
    - Kantholz wird nur unten angezeigt.
    - Einlagen werden nur als echte Zwischenlagen angezeigt.
    """
    columns = [
    ...
    # Echte Verladeeinheiten/Bunde: nicht in Einzelbauteile zerlegen.
    for _, row in rows.sort_values(['Z_mm', 'X_mm', 'Y_mm'], kind='stable').iterrows():
        ...
        label = _format_label_value(row.get('Ansicht_Label'))
        if not label:
            label = _format_bsd_cell(row)

        count = max(1, int(safe_number(row.get('Anzahl_Bauteile'), 1)))
        if typ == 'Bund' and count > 1:
            # Kurze, eindeutige Bunddarstellung statt künstlicher Einzellagen.
            label = f"{label}\n({count} Stk.)"
    ...
    # Einlage-Zeilen nur zwischen echten Verladeeinheiten/Lagen.
    ...
    existing_real_keys = {(round(e['z'], 1), e['slot']) for e in entries if e['kind'] not in ['Kantholz erste Lage']}
    for z_val in layer_z_values:
        layer_rows = rows[rows['Z_mm'].round(1) == z_val].copy()
        for _, lrow in layer_rows.iterrows():
            if str(lrow.get('Typ', '')).strip() == 'Unterbau':
                continue
    ...
    # Einlage/Lagenholz nie in dieselbe Zeile wie eine echte Verladeeinheit mischen.
    real_z_levels = {round(e['z'], 1) for e in entries if e['kind'] not in ['Kantholz erste Lage'] and 'Einlage' not in e['kind'] and 'Lagenholz' not in e['kind']}
    for e in entries:
    ...
    for z_key in display_levels:
        ...
        out: Dict[str, Any] = {
            ...
            'Anzahl_Einheiten': int(sum(1 for e in level_entries if 'Einlage' not in e['kind'] and 'Lagenholz' not in e['kind'] and e['kind'] != 'Kantholz erste Lage')),
            'Zeilentyp': ', '.join(sorted({e['kind'] for e in level_entries})),
        }

        real_units = [e for e in level_entries if 'Einlage' not in e['kind'] and 'Lagenholz' not in e['kind'] and e['kind'] != 'Kantholz erste Lage']
        if real_units:
            out['Breite_mm'] = round(max((e['width'] for e in real_units), default=0.0), 1)
            out['Gesamtlänge_mm'] = round(max((e['length'] for e in real_units), default=0.0), 1)
        else:
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v50')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v50') if option_choices_manual else ''
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v50', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v50')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v50'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v50'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v50'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v50', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v50', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v50')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v50')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v50',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v50', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v50', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v50',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v50')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v50', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v50', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v50', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v50'] = validation_df
                            st.session_state['manual_plan_ready_v50'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v50'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v50'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v50', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v50',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v50')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v50', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v50_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v50', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Die Tabelle wird automatisch für jede belegte Pritsche erstellt. V50: gleiche Verladeeinheiten/Bunde wie in der Visualisierung; keine künstliche Zerlegung in Einzelbauteil-Lagen.')
