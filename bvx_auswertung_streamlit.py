...
def create_bsd_matrix_for_platform(
    ...
    - Keine starre Drittel-Logik: Mitte entsteht nur bei echten 3 X-Gruppen je Lage/Seite.
    - Die Tabelle ist unten ausgerichtet: unten ist unten, Kantholz bleibt unten.
    - Jede Positionsspalte ist ein eigener Stapel; es gibt keine gemeinsame Höhenmatrix.
    ...
    def _build_x_zone_lookup() -> Dict[Tuple[int, str], str]:
        """Mitte wird nur aus echten X-Gruppen je Lage/Seite abgeleitet."""
        ...
        for (z_key, side), items in groups.items():
    ...
    # Je Spalte physisch sortieren: unten -> oben; Anzeige dann oben -> unten mit Bottom-Alignment.
    ...
    for slot in position_cols:
        ...
        slot_entries = sorted(slot_entries, key=lambda e: (round(float(e.get('z', 0.0)), 1), str(e.get('label', ''))), reverse=False)
    ...
    for display_idx in range(max_len):
        # display_idx 0 = oberste Tabellenzeile. Pro Spalte werden oben Leerfelder ergänzt.
        out: Dict[str, Any] = {
            'Pritsche': pname,
            'Fuhre_Nr': platform.get('Fuhre_Nr', ''),
            ...
            'Gewicht_kg': '',
            'Anzahl_Einheiten': '',
            'Zeilentyp': 'BSD-6-Spalten-unten-ausgerichtet',
        }
        ...
        for slot in position_cols:
            stack = slot_lists_bottom_to_top[slot]
            top_pad = max_len - len(stack)
            local_idx = display_idx - top_pad
            if local_idx < 0 or local_idx >= len(stack):
                continue
            # stack ist unten->oben, local_idx 0 soll unten sein. Anzeige oben->unten = reverse Zugriff.
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v57')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v57') if option_choices_manual else ''
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v57', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v57')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v57'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v57'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v57'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v57', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v57', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v57')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v57')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v57',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v57', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v57', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v57',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v57')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v57', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v57', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v57', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v57'] = validation_df
                            st.session_state['manual_plan_ready_v57'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v57'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v57'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v57', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v57',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v57')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v57', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v57_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v57', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Die Tabelle wird automatisch für jede belegte Pritsche erstellt. V57: 6 Positionsspalten, unten ausgerichtet; Kantholz unten, leere Felder nur oben.')
