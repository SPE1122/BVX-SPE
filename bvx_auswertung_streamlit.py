...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v39')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v39') if option_choices_manual else ''
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            st.caption('Pritsche wählen, Bauteile/Bunde anhaken, Position anklicken/auswählen. Die Lage wird automatisch auf der bestehenden Auflage gesetzt.')
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche wählen', platform_names, key='manual_simple_platform_v39')
                show_sketch = top2.checkbox('Skizze anzeigen', value=False, key='manual_simple_show_sketch_v39')
                if top3.button('Zurücksetzen', key='manual_simple_reset_v39'):
                    manual_init = clean_placements_dataframe(placements_df)
                ...
                with st.expander('Pritschenwerte ändern', expanded=False):
                    ...
                    edited_platform = st.data_editor(
                        ...
                        hide_index=True,
                        num_rows='fixed',
                        key='manual_platform_values_v39',
                    )
                ...
                if work_df.empty:
                    ...
                    search_text = st.text_input('Suche Bauteil/Bund', value='', key='manual_search_v39')
                    ...
                    display_cols = [
                        ...
                        'Ziel_Pritsche', 'Position',
                        'Einlage_unten_mm', 'Einlage_oben_mm',
                        'Länge_mm', 'Breite_mm', 'Höhe_mm', 'Gewicht_kg'
                    ]
                    display_cols = [c for c in display_cols if c in editor.columns]

                    with st.form('manual_batch_form_v39'):
                        edited_batch = st.data_editor(
                            editor[display_cols],
                            use_container_width=True,
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_batch_editor_v39',
                            column_config={
                    ...
                    if apply_manual:
                        ...
                        for _, erow in edited_batch.iterrows():
                            ...
                            manual_df.at[idx, 'Manuelle_Lage'] = 'automatisch auf bestehende Lage'
                            manual_df.at[idx, 'Z_manuell_mm'] = safe_number(manual_df.at[idx, 'Z_mm'], 0.0)
                            changed += 1
                ...
                if show_sketch:
                    ...
                    st.plotly_chart(
                        ...
                        use_container_width=True,
                        key=f'manual_plot_v39_{selected_manual_platform}_{view_map[view_choice]}',
                    )
                else:
