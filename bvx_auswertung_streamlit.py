        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            if matrix is not None and not matrix.empty:
                for i, (_, mrow) in enumerate(matrix.iterrows(), start=write_start):
                    row_values = [
                        mrow.get('Hinten links', ''), mrow.get('Mitte links', ''), mrow.get('Vorne links', ''),
                        mrow.get('Hinten rechts', ''), mrow.get('Mitte rechts', ''), mrow.get('Vorne rechts', ''),
                    ]
...
def create_bsd_matrix_for_platform(
    ...
    for _, row in rows.sort_values(['Z_mm', 'X_mm', 'Y_mm'], kind='stable').iterrows():
        ...
        for i, label in enumerate(labels_bottom_to_top):
            ...
            if i < count - 1 and internal_spacer > 0:
                for slot in slots_for_row:
                    add_entry(z_cursor, slot, _fmt_bsd_mm_label('Einlage', internal_spacer), 'Einlage', internal_spacer, row['Länge_mm'], row['Breite_mm'], 0.0, '')
    ...
    # V64: geometrischer Abgleich für die BSD-Darstellung.
...
def _bsd_is_visible_simple_entry(value: Any) -> bool:
    """Einlage nicht als eigener sichtbarer BSD-Eintrag.

    Einlagen werden im einfachen BSD nur als Hinweistext erwähnt.
    Dadurch zerschneiden sie keine Bunde mehr optisch.
    """
    txt = str(value or '').strip()
    if not txt:
        return False
    low = txt.lower()
    if low.startswith('einlage') or low.startswith('lagenholz'):
        return False
    return True


def _bsd_compact_column_lists(matrix_df: pd.DataFrame) -> Dict[str, List[str]]:
    """Kompakte BSD-Listen je Spalte.

    Keine Z-Abstände, keine Einlage-Zeilen.
    Reihenfolge wird aus der BSD-Matrix übernommen, aber je Spalte kompakt zusammengezogen.
    """
    position_cols = ['Hinten links', 'Mitte links', 'Vorne links', 'Hinten rechts', 'Mitte rechts', 'Vorne rechts']
    compact: Dict[str, List[str]] = {col: [] for col in position_cols}
    ...
    for col in position_cols:
        ...
        vals: List[str] = []
        seen_tail: List[str] = []
        for value in matrix_df[col].tolist():
            text_value = str(value or '').strip()
            if not text_value:
                continue
            for part in text_value.split('\n'):
                part = str(part or '').strip()
                if not _bsd_is_visible_simple_entry(part):
                    continue
                vals.append(part)
        compact[col] = vals
    return compact
...
def format_bsd_simple_text_matrix(matrix_df: pd.DataFrame) -> str:
    """Erzeugt eine kompakte BSD-Textmatrix ohne Einlage-Zeilen und ohne Z-Abstände."""
    position_cols = ['Hinten links', 'Mitte links', 'Vorne links', 'Hinten rechts', 'Mitte rechts', 'Vorne rechts']
    compact = _bsd_compact_column_lists(matrix_df)
    max_len = max((len(v) for v in compact.values()), default=0)
    if max_len <= 0:
        return ''
    ...
    for i in range(max_len):
        vals: List[str] = []
        for col in position_cols:
            value = compact.get(col, [])[i] if i < len(compact.get(col, [])) else ''
            vals.append(str(value)[:col_width - 1].ljust(col_width))
        lines.append(''.join(vals).rstrip())

    lines.append('')
    lines.append('Hinweis: Einlage 40 / Lagenholz gemäss Einstellung zwischen den Bunden einlegen.')
    lines.append('BSD ist eine kompakte Leseliste; echte Geometrie/Höhe steht im Lageplan.')
    return '\n'.join(lines)
...
def _pdf_draw_bsd_matrix_page(c, page_w: float, page_h: float, margin: float, platform: pd.Series, matrix_df: pd.DataFrame, header: Dict[str, Any], project_name: str, logo_bytes: Optional[bytes] = None) -> None:
    ...
    c.drawString(left_x + 8, top_y - 120, 'Darstellung: kompakte BSD-Leseliste. Einlagen nur als Hinweis, keine Z-Abstände.')
    ...
    # Kompakte BSD-Leseliste:
    # je Spalte eigene Liste, keine Z-Abstände, keine Einlage-Zeilen.
    compact_cols = _bsd_compact_column_lists(rows)
    max_len = max((len(v) for v in compact_cols.values()), default=0)
    if max_len <= 0:
        c.setFont('Helvetica', 8)
    ...
    row_h = min(15.5, max(8.5, usable_h / max(max_len, 1)))
    font_size = 7.2 if row_h >= 12 else 6.2

    # Spaltenkopf und vertikale Begrenzungen, aber keine horizontalen Zeilenlinien.
    ...
    def _draw_cell_text(x0: float, y0: float, w0: float, txt: str) -> None:
        ...
        max_chars = max(8, int(w0 / (font_size * 0.55)))
        if len(txt) > max_chars:
            txt = txt[:max_chars - 1] + '…'
        c.drawString(x0 + 3, y0, txt)

    # Top-down kompakt je Spalte. Keine leeren Z-Zeilen.
    for ci, col in enumerate(position_cols):
        x0 = area_x + ci * (col_w + gap)
        vals = compact_cols.get(col, [])
        y = area_top - header_h - row_h + 4
        for val in vals:
            if y < area_bottom:
                break
            _draw_cell_text(x0, y, col_w - 6, val)
            y -= row_h

    c.setFillColor(colors.black)
    c.setFont('Helvetica', 6.4)
    c.drawString(margin, 24, 'Hinweis: Einlage 40 / Lagenholz gemäss Einstellung zwischen den Bunden einlegen.')
    c.drawString(margin, 14, 'BSD ist eine kompakte Leseliste. Die echte Geometrie, Auflage und Höhe ist im Lageplan massgebend.')
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v64')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v64') if option_choices_manual else ''
    ...
    if start_col1.button('Verladung starten / neu berechnen', type='primary', key='start_loading_plan_v64'):
        st.session_state['loading_plan_started_v64'] = True
    ...
    if start_col2.button('Verladung zurücksetzen', key='reset_loading_plan_v64'):
        st.session_state['loading_plan_started_v64'] = False
        for k in ['manual_plan_signature', 'manual_platform_signature', 'manual_placements_df', 'manual_platforms_df']:
            st.session_state.pop(k, None)
        st.rerun()

    if not bool(st.session_state.get('loading_plan_started_v64', False)):
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v64', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v64')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v64'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v64'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v64'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v64', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v64', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v64')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v64')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v64',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v64', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v64', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v64',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v64')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v64', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v64', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v64', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v64'] = validation_df
                            st.session_state['manual_plan_ready_v64'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v64'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v64'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v64', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v64',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v64')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v64', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v64_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v64', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Kompakte BSD-Leseliste: keine Einlage-Zeilen, keine Z-Abstände, Einlagen nur als Hinweistext.')
