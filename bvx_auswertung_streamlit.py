...
def create_bsd_matrix_for_platform(
    ...
    V54:
    - Grundlage bleibt exakt die vorhandene X/Y/Z-Platzierung der Visualisierung.
    - Eine echte Z-Lage bleibt eine Tabellenzeile.
    - Leere Zonen bleiben leer, nicht mit Platzhaltern gefüllt.
    - Kantholz wird nur unten angezeigt.
    - Einlagen werden nur als eigene echte Zwischenlagen angezeigt.
    """
    columns = [
    ...
    def _bsd_x_zone(row: pd.Series) -> str:
        """Zusatzkennzeichnung innerhalb der 4 BSD-Spalten: H / M / V."""
        length = safe_number(row.get('Länge_mm'))
        x0 = safe_number(row.get('X_mm'))
        if eff_length <= 0 or length <= 0:
            return ''
        x_mid = x0 + length / 2.0
        back_limit = eff_length / 3.0
        front_limit = eff_length * 2.0 / 3.0
        if x_mid < back_limit:
            return 'H'
        if x_mid > front_limit:
            return 'V'
        return 'M'

    def _set_last_entry_x_zone(x_zone: str, row: Optional[pd.Series] = None) -> None:
        if not entries:
            return
        entries[-1]['x_zone'] = str(x_zone or '').strip()
        if row is not None:
            length = safe_number(row.get('Länge_mm'))
            x0 = safe_number(row.get('X_mm'))
            entries[-1]['x_mid'] = x0 + length / 2.0 if length > 0 else None
    ...
    for _, row in rows.sort_values(['Z_mm', 'X_mm', 'Y_mm'], kind='stable').iterrows():
        ...
        if typ == 'Unterbau':
            ...
            for slot in slots_for_row:
                ...
                _set_last_entry_x_zone(x_zone, row)
            continue
        ...
        for i, label in enumerate(labels_bottom_to_top):
            ...
            for slot in slots_for_row:
                add_entry(z_cursor, slot, visible_label, 'Bund-Bauteil', ph, pl, pw, part_weight, remark)
                _set_last_entry_x_zone(x_zone, row)
            z_cursor += ph
    ...
    # H/M/V-Kennzeichnung erst nach dem Aufbau der echten BSD-Zeilen vergeben.
    # Wichtig: M gibt es nur, wenn auf gleicher Höhe und gleicher Seite mindestens
    # drei reale Einträge/Bunde in X-Richtung existieren. Sonst bleibt die alte
    # vorne/hinten-Spaltenlogik ohne Zusatzkennzeichnung.
    def _assign_hmv_only_for_three_on_same_side() -> None:
        for e in entries:
            if ('Bauteil' in str(e.get('kind', ''))) or str(e.get('kind', '')) == 'Unterbau':
                e['x_zone'] = ''
        ...
        for _, group in grouped.items():
            # Mehrere Einträge mit gleicher X-Mitte zählen als ein Bereich.
            ...
            for e in group:
                x_mid = round(float(e.get('x_mid')), 1)
                if abs(x_mid - back_pos) <= 1.0:
                    e['x_zone'] = 'H'
                elif abs(x_mid - front_pos) <= 1.0:
                    e['x_zone'] = 'V'
                else:
                    e['x_zone'] = 'M'

    _assign_hmv_only_for_three_on_same_side()
    ...
    for z_key in display_levels:
        ...
        for slot in slots:
            ...
            visible_entries = [e for e in slot_entries if e['label']]
            real_visible = [
                e for e in visible_entries
                if e['kind'] not in ['Kantholz erste Lage']
                and 'Einlage' not in e['kind']
                and 'Lagenholz' not in e['kind']
            ]
            show_x_zone = any(str(e.get('x_zone', '') or '').strip() for e in real_visible)
            zone_order = {'H': 0, 'M': 1, 'V': 2, '': 3}
            if show_x_zone:
                visible_entries = sorted(
                    visible_entries,
                    key=lambda e: (zone_order.get(str(e.get('x_zone', '') or '').strip(), 9), safe_number(e.get('x_mid'), 0.0), str(e.get('label', '')))
                )
            cell_lines = []
            for e in visible_entries:
                lbl = e['label']
                zone = str(e.get('x_zone', '') or '').strip()
                if show_x_zone and zone and e in real_visible:
                    lbl = f"{zone}: {lbl}"
                cell_lines.append(lbl)
            out[slot] = (' / '.join(cell_lines) if show_x_zone else '\n'.join(cell_lines))
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v54')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v54') if option_choices_manual else ''
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v54', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v54')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v54'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v54'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v54'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v54', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v54', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v54')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v54')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v54',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v54', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v54', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v54',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v54')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v54', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v54', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v54', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v54'] = validation_df
                            st.session_state['manual_plan_ready_v54'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v54'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v54'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v54', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v54',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v54')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v54', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v54_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v54', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        ...
        st.caption('Die Tabelle wird automatisch für jede belegte Pritsche erstellt. V54: Bundnummern bleiben; H/M/V nur bei drei X-Bereichen auf gleicher Lage/Seite, in einer Zellzeile mit / getrennt.')
