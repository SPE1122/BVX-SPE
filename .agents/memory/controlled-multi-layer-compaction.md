...
When two narrow units from adjacent layers should share a level, evaluate the complete local unloading chain rather than moving only the pair. A valid pattern can place narrow–wide–narrow units on the lower cross-layer and the intervening wide unit above only its logically later supports; direct upper dependents join the same atomic validation.
...
**How to apply:** Detect consecutive, equal-height local windows by rank and dimensions rather than part numbers. Revalidate every changed support at the configured minimum; allow only a small bounded lateral center-of-gravity tradeoff when height is reduced, while longitudinal balance must not worsen.
