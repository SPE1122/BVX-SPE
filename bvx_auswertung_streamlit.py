...
def create_loading_excel(
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        def add_styled_bsd_sheet(header: pd.Series, matrix: pd.DataFrame):
            ...
            # V78: BSD-Matrix A:F breiter für bessere Lesbarkeit auf Ausdruck.
            widths = {
...
def create_bsd_matrix_for_platform(
    ...
    # V78: geometrischer Abgleich für die BSD-Darstellung.
...
def _bsd_display_matrix_rows(matrix_df: pd.DataFrame) -> Tuple[List[str], List[List[str]]]:
    ...
    Ziel: Bundblock + zugehörige Einlage bleiben zusammen und werden nicht durch
    andere Tabellenzeilen auseinandergezogen. Keine Änderung an Verladung oder
    Geometrie.
    """
...
def render_loading_module(uploaded_file, transport_excel_file=None, logo_file=None, parts_excel_file=None) -> None:
    ...
    verladeart = st.radio('Verladeart wählen', ['Automatisch', 'Manuell'], horizontal=True, key='verladeart_v78')
    ...
    if verladeart == 'Manuell':
        ...
        manual_fuhrenoption = st.selectbox('Fuhrenoption für manuelle Verladung', option_choices_manual, key='manual_fuhrenoption_v78') if option_choices_manual else ''
    ...
    if start_col1.button('Verladung starten / neu berechnen', type='primary', key='start_loading_plan_v78'):
        st.session_state['loading_plan_started_v78'] = True
    ...
    if start_col2.button('Verladung zurücksetzen', key='reset_loading_plan_v78'):
        st.session_state['loading_plan_started_v78'] = False
        for k in ['manual_plan_signature', 'manual_platform_signature', 'manual_placements_df', 'manual_platforms_df']:
            st.session_state.pop(k, None)
        st.rerun()

    if not bool(st.session_state.get('loading_plan_started_v78', False)):
    ...
    # V78:
    # Die final korrigierte Platzierung muss für Pritschenplan UND BSD gleich sein.
    # Darum wird die manuelle Session-Signatur versioniert. So können keine alten
    # manual_placements_df aus einer vorherigen App-Version weiterlaufen und in der
    # BSD-Tabelle den alten Versatz zeigen.
    plan_state_version = 'v78_stabile_x_und_bsd_sync'
    manual_signature = f"{plan_state_version}|{verladeart}|{manual_fuhrenoption}|" + _manual_plan_signature(placements_df)
    platform_signature = f"{plan_state_version}|{verladeart}|{manual_fuhrenoption}|" + _manual_platform_signature(platforms_used_df)
    if (
    ...
    manual_plan_ready = bool(st.session_state.get('manual_plan_ready_v78', False)) if verladeart == 'Manuell' else True
    ...
    with tab3:
        if verladeart == 'Automatisch':
            ...
            if not platform_names:
                ...
                selected_manual_platform = top1.selectbox('Pritsche', platform_names, key='manual_zone_platform_v78')
                if top2.button('Neue Pritsche starten', key='manual_start_next_platform_v78'):
                    platforms_used_df = _manual_start_next_fuhre(platforms_used_df)
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.session_state['manual_plan_ready_v78'] = False
                    st.rerun()
                if top3.button('Reset', key='manual_zone_reset_v78'):
                    manual_init = clean_placements_dataframe(placements_df)
                    manual_init['Einlage_unten_mm'] = 0.0
                    manual_init['Einlage_oben_mm'] = 0.0
                    manual_init['Manuelle_Zone'] = ''
                ...
                if s2.button('Pritsche abschliessen', key='manual_close_platform_v78', disabled=(platform_status == 'Abgeschlossen')):
                    platforms_used_df = _manual_set_platform_status(platforms_used_df, selected_manual_platform, 'Abgeschlossen')
                    st.session_state['manual_platforms_df'] = platforms_used_df
                    st.rerun()
                if s3.button('Pritsche wieder öffnen', key='manual_open_platform_v78', disabled=(platform_status != 'Abgeschlossen')):
                ...
                with c_left:
                    ...
                    search_text = st.text_input('Suche', value='', key='manual_zone_search_v78')
                    ...
                    if work_df.empty:
                        ...
                        selected_label = st.selectbox('Auswahl', labels, key='manual_zone_unit_v78')
                        ...
                        spacer_under = st.number_input(
                            ...
                            step=5.0,
                            key='manual_zone_spacer_v78',
                        )

                        if st.button('ausgewähltes Element entladen / rückgängig', key='manual_zone_unload_v78', disabled=platform_locked):
                        ...
                        for col_obj, zone_code, label in direct_buttons:
                            if col_obj.button(label, key=f'manual_zone_direct_{zone_code}_v78', disabled=platform_locked):
                    ...
                    with st.expander('Pritschenwerte kurz ändern', expanded=False):
                        ...
                        edited_platform = st.data_editor(
                            ...
                            hide_index=True,
                            num_rows='fixed',
                            key='manual_zone_platform_values_v78',
                        )
                        platform_changed = (
                ...
                with c_right:
                    ...
                    if not loaded_on_platform.empty:
                        ...
                        undo_choice = u1.selectbox('verladenes Element rückgängig', undo_labels, key='manual_zone_undo_choice_v78')
                        if u2.button('rückgängig', key='manual_zone_undo_button_v78', disabled=platform_locked):
                    ...
                    ready_txt = 'berechnet' if bool(st.session_state.get('manual_plan_ready_v78', False)) else 'noch nicht berechnet'
                    st.write(f'Verladen: {loaded_now} · offen: {free_now} · Status: {ready_txt}')

                    if st.button('Ladeplan berechnen / prüfen', key='manual_zone_calculate_plan_v78', type='primary'):
                        ...
                        if validation_df is not None and not validation_df.empty:
                            st.session_state['manual_validation_issues_v78'] = validation_df
                            st.session_state['manual_plan_ready_v78'] = False
                            ...
                            st.session_state['manual_platforms_df'] = platforms_used_df
                            st.session_state['manual_validation_issues_v78'] = pd.DataFrame()
                            st.session_state['manual_plan_ready_v78'] = True
                            st.rerun()

                    validation_show = st.session_state.get('manual_validation_issues_v78', pd.DataFrame())
                    ...
                    with st.expander('Ansicht prüfen', expanded=False):
                        ...
                        view_choice = st.radio(
                            ...
                            horizontal=True,
                            key='manual_zone_view_v78',
                        )
                        create_view = st.button('Ansicht erstellen / aktualisieren', key='manual_zone_create_view_v78')
                        if create_view:
                            if not bool(st.session_state.get('manual_plan_ready_v78', False)):
                            ...
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f'manual_zone_plot_v78_{selected_manual_platform}_{view_map[view_choice]}',
                            )
                        else:
                ...
                if bool(st.session_state.get('manual_plan_ready_v78', False)):
                    edited_summary_df = recompute_summary_from_placements(edited_placements_df, platforms_used_df) if not platforms_used_df.empty else summary_df
                    warnings_plan_df = compute_loading_warnings(edited_placements_df, platforms_used_df)
                    display_placements_df = edited_placements_df.copy()
    ...
    with tab4:
        # V78:
        # BSD wird ausdrücklich aus derselben finalen Platzierung erzeugt
        # wie Pritschenplan / Ansichten / Export.
        final_bsd_placements_df = clean_placements_dataframe(display_placements_df)
        bsd_header_df, bsd_matrix_df = create_all_bsd_matrices(
            final_bsd_placements_df,
            platforms_used_df,
            edited_summary_df,
            warnings_plan_df,
    ...
    with tab7:
        ...
        # V78: Auch der Export nimmt dieselbe finale Grundlage wie der Pritschenplan.
        final_bsd_placements_df = clean_placements_dataframe(display_placements_df)
        if 'bsd_header_df' not in locals() or 'bsd_matrix_df' not in locals():
            bsd_header_df, bsd_matrix_df = create_all_bsd_matrices(final_bsd_placements_df, platforms_used_df, edited_summary_df, warnings_plan_df, project_meta=project_meta)
    ...
    with st.sidebar:
        ...
        if module == 'BVX Auswertung':
            ...
            logo_file = st.file_uploader(
                ...
                type=['png', 'jpg', 'jpeg'],
                key='logo_upload',
                help='Ohne Upload wird in den Dokumenten kein Logo platziert. Das App-Logo in der Seitenleiste bleibt davon getrennt.'
            )
            analysis_file = None
            if loading_file:
